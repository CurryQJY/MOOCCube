param(
    ...
    [string]$OutputRoot = "outputs\usim_feedback_fast3_standalone_redundant_compare",
    [string]$CheckpointRoot = "checkpoints\usim_feedback_fast3_standalone_redundant_compare",
    [switch]$ForceCPU,
...
$experiments = @(
    (New-Experiment "concept_only_no_redundant" $conceptOnlyBase "Standalone concept reward only baseline with redundant disabled."),
    (New-Experiment "concept_only_old_redundant" (Merge-Hashtables @(
        $conceptOnlyBase,
        @{
            "USIM_FB_COURSE_REDUNDANT_W" = "0.02"
            "USIM_FB_COURSE_REDUNDANT_MODE" = "concept"
        }
    )) "Standalone concept reward + original concept-overlap redundant."),
    (New-Experiment "concept_only_new_redundant" (Merge-Hashtables @(
        $conceptOnlyBase,
        @{
            "USIM_FB_COURSE_REDUNDANT_W" = "0.02"
    ...
    )) "Standalone concept reward + new structural redundant based on same-family plus video containment.")
)
...
try {
    ...
    $summaryPath = Join-Path $OutputRoot "summary_redundant_compare.csv"
