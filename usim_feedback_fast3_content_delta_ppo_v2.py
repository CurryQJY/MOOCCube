...
def _compose_ppo_distance_reward(
    new_distance,
    step_gain,
    step_idx,
    num_steps,
    ...
    """Dense progress reward plus a terminal distance penalty on the last step."""
    reward = float(gain_weight) * step_gain
    if int(step_idx) == int(num_steps) - 1:
        reward = reward - float(terminal_weight) * new_distance
    return reward
...
class SimpleAC(nn.Module):
    ...
    def get_action_value(self, item_state, time_step, candidates_emb, action_idx=None):
        ...
        if action_idx is None:
            action_idx = dist.sample() if self.training else logits.argmax(dim=-1)
    ...
    def run_usim_episode(
        ...
        for t in range(self.cfg.usim_steps):
            ...
            if target_emb is not None:
                ...
                reward = _compose_ppo_distance_reward(
                    new_dist,
                    step_gain,
                    step_idx=t,
                    num_steps=self.cfg.usim_steps,
                    terminal_weight=self.cfg.reward_terminal_weight,
                    gain_weight=self.cfg.reward_gain_weight,
                )
