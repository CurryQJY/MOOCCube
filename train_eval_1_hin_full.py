...
class PAM_Enhanced(nn.Module):
    ...
    def forward(self, batch):
        ...
        # 2. Item Vector
        z_i, id_e_raw, content_e = self.get_item_vector(i_idx, force_cold=False)
...
def evaluate_pam(model, loader, device, k_list=[5, 10, 20], n_neg=999, eval_type='cold', full_ranking=False):
    """
    评估函数。
    ...
    with torch.no_grad():
        ...
        for start in range(0, n_items, ITEM_BATCH):
            ...
            z_batch, _, _ = model.get_item_vector(idx_batch, force_cold=use_cold)
...
def main():
    print("Loading Data for PAM Enhanced...")
    print("Loading Data for PAM Enhanced (HIN)...")
    ...
    for t in range(len(periods)):
        ...
        if t >= WARMUP_PERIODS:
            # 采样评估
            met_cold, n_cold_t = evaluate_pam(model, eval_loader, device, k_list, eval_type='cold')
            met_hot, n_hot_t = evaluate_pam(model, eval_loader, device, k_list, eval_type='hot')
            met_all, n_all_t = evaluate_pam(model, eval_loader, device, k_list, eval_type='all')
            # 全库排名评估
            fmet_cold, fn_c = evaluate_pam(model, eval_loader, device, k_list, eval_type='cold', full_ranking=True)
            fmet_hot, fn_h = evaluate_pam(model, eval_loader, device, k_list, eval_type='hot', full_ranking=True)
            fmet_all, fn_a = evaluate_pam(model, eval_loader, device, k_list, eval_type='all', full_ranking=True)
    ...
    pd.DataFrame(history).to_csv('mooc_metrics_pam_final_hin_full.csv', index=False)
    ...
    plt.title('PAM Enhanced HIN: Cumulative Training')
    plt.legend()
    plt.grid(True, alpha=0.5)
    plt.savefig('mooc_result_pam_final_hin_full.png')
    print(">> Saved mooc_result_pam_final_hin_full.png and csv")
