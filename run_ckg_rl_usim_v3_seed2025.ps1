...
if ($Seed -ne 2025) {
    throw "This V3 acceptance launcher is pinned to seed 2025. Create a separate launcher for another seed."
}
...
$runName = "seed$Seed"
$outputRelative = Join-Path "outputs\ckg_rl_usim_v3" $runName
$checkpointRelative = Join-Path "checkpoints\ckg_rl_usim_v3" $runName
...
if (-not $DryRun -and ((Test-Path -LiteralPath $outputRoot) -or (Test-Path -LiteralPath $checkpointRoot))) {
    throw "Refusing to overwrite an existing V3 run. Use a new isolated run root instead."
}
...
# USIM_ORIGINAL_V2 only activates its frozen-teacher/pseudo-cold plumbing; the
# V3 wrapper replaces the episode and PPO implementations entirely.
$lockedEnvironment = @{
    "USIM_ORIGINAL_V2" = "1"
    "USIM_V3_CORE" = "1"
...
try {
    ...
    if ($DryRun) {
        $contractFormat = "CKG-RL V3 contract: teacher={0} pseudo_mode=item_tail pseudo_ratio={1} " +
            "step={2} candidates={3} ckg_reward=on prereq_aux=on"
        Write-Host ($contractFormat -f $teacherCheckpointPath, $PseudoColdRatio, $lockedEnvironment["USIM_V3_STEP_SIZE"], $Candidates)
        $params["DryRun"] = $true
    }
