...
class FullRankCourseConfig(CourseConfig):
    def __init__(self, n_users, n_items, content_dim=768):
        super().__init__(n_users, n_items, content_dim)
        self.fullrank_force_cold_items = os.environ.get("USIM_FULLRANK_FORCE_COLD_ITEMS", "1") == "1"
        self.fullrank_item_batch = int(os.environ.get("USIM_FULLRANK_ITEM_BATCH", "1024"))
        self.fullrank_teacher_weight = float(os.environ.get("USIM_FULLRANK_TEACHER_WEIGHT", "0.00"))
        self.fullrank_teacher_top_m = int(os.environ.get("USIM_FULLRANK_TEACHER_TOPM", "8"))
        self.fullrank_teacher_temp = float(os.environ.get("USIM_FULLRANK_TEACHER_TEMP", "0.20"))
        self.fullrank_teacher_concept_alpha = float(os.environ.get("USIM_FULLRANK_TEACHER_CONCEPT_ALPHA", "0.10"))
        self.fullrank_teacher_only_cold = os.environ.get("USIM_FULLRANK_TEACHER_ONLY_COLD", "1") == "1"


class FullRankCourseUSIM(CourseAwareUSIM):
    ...
    def build_course_item_bank_grad(self, force_cold=True, item_batch=1024):
        all_vecs = []
    ...
    def _build_teacher_proto(self, anchor_vec, item_idx):
        if self.warm_item_idx is None or self.warm_item_idx.numel() < 1:
            return torch.zeros_like(anchor_vec)

        with torch.no_grad():
            retrieve_bank = self.build_course_item_bank(
                force_cold=True,
                item_batch=self.cfg.fullrank_item_batch,
                deterministic=True,
            )
            warm_retrieve = retrieve_bank[self.warm_item_idx]
            warm_teacher = F.normalize(self.item_id_emb(self.warm_item_idx).detach(), dim=1)

            scores = torch.matmul(anchor_vec.detach(), warm_retrieve.t())
            concept_alpha = float(self.cfg.fullrank_teacher_concept_alpha)
            if concept_alpha > 0.0 and self.item_concept_overlap is not None:
                scores = scores + concept_alpha * self.item_concept_overlap[item_idx][:, self.warm_item_idx]

            if self.warm_pos_map is not None:
                self_pos = self.warm_pos_map[item_idx]
                valid_rows = self_pos >= 0
                if valid_rows.any():
                    scores[valid_rows, self_pos[valid_rows]] = -1e9

            top_m = min(max(1, int(self.cfg.fullrank_teacher_top_m)), int(self.warm_item_idx.numel()))
            temp = max(float(self.cfg.fullrank_teacher_temp), 1e-6)
            top_scores, top_pos = torch.topk(scores, k=top_m, dim=1)
            weights = F.softmax(top_scores / temp, dim=1)
            proto = (warm_teacher[top_pos] * weights.unsqueeze(-1)).sum(dim=1)
...
def run_static_experiment_fullrank(df, cfg, device, model, optimizer, llm_scores):
    ...
    print(
        f"\n>>> Start STATIC fullrank train/eval | split={train_ratio:.2f}/{val_ratio:.2f}/{1.0 - train_ratio - val_ratio:.2f} "
    ...
    for epoch in range(cfg.n_epochs):
        ...
        if avg_dup is not None:
            print(
                f"  [STATIC-FULLRANK] Epoch {epoch + 1}/{cfg.n_epochs} | Loss: {avg_loss:.4f} | "
    ...
    print("\n" + "=" * 90)
    print(f"         FINAL REPORT (STATIC FULLRANK): sampled (1+{cfg.eval_n_neg}) vs full ranking")
    print("=" * 90)
...
def main():
    ...
    print(f"Loading Data for Course FullRank USIM from {data_dir}...")
    ...
    cfg = FullRankCourseConfig(meta["n_users"], meta["n_items"], content_emb.shape[1])
    ...
    model = FullRankCourseUSIM(cfg, content_emb).to(device)
    ...
    print(f">> Architecture: Course FullRank USIM (Batch Size={cfg.batch_size})")
    print(
    ...
    print(
        ...
        f"teacher_w={cfg.fullrank_teacher_weight:.2f} | topM={cfg.fullrank_teacher_top_m} | "
        f"temp={cfg.fullrank_teacher_temp:.2f} | concept_alpha={cfg.fullrank_teacher_concept_alpha:.2f} | "
        f"teacher_only_cold={cfg.fullrank_teacher_only_cold}"
    )
    print(
    ...
    if os.environ.get("USIM_STATIC", "0") == "1":
        run_static_experiment_fullrank(df, cfg, device, model, optimizer, llm_scores)
        return

    periods = split_dataframe_by_periods(df, period_type="M")
    print(f"\n>>> Start cumulative fullrank train/eval - total {len(periods)} periods <<<")
    ...
    for t, p_df in enumerate(periods):
        ...
        for epoch in range(cfg.n_epochs):
            ...
            if avg_dup is not None:
                print(
                    f"  [TRAIN-FULLRANK] Epoch {epoch + 1}/{cfg.n_epochs} | cumulative: {len(combined_df)} | "
    ...
    print("\n" + "=" * 90)
    print(f"         FINAL REPORT: sampled (1+{cfg.eval_n_neg}) vs full ranking (Course FullRank USIM)")
    print("=" * 90)
    print(f"{'Metric':<10} | {'Sampled Cold':<12} | {'Sampled Hot':<12} | {'Full Cold':<12} | {'Full Hot':<12}")
    print("-" * 90)
    for key in metrics_keys:
    ...
    pd.DataFrame(history).to_csv("mooc_metrics_course_usim_fullrank.csv", index=False)
    ...
    plt.title("Course FullRank USIM: Cumulative Training")
    plt.legend()
    plt.tight_layout()
    plt.savefig("mooc_result_course_usim_fullrank.png")
