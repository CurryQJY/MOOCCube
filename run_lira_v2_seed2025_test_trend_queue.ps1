...
$queueRoot = "background_logs\lira_v2_seed2025_test_trend"
$outputBase = "outputs\learner_guided_full_testtrend"
...
$trainingArms = @(
    "ablation_t0", "ablation_t1", "ablation_no_stop",
    "ablation_no_refined_loss", "ablation_no_stability"
)
$trainingCheckpoints = @()
foreach ($arm in $trainingArms) {
    foreach ($seed in 2026,2027) {
        $trainingCheckpoints += "checkpoints\learner_guided_full\lira_v2_$($arm)_seed$seed\strict_item_cold_balanced_thr1_seed_$seed\validation_finished.pt"
    }
}

Write-QueueLog "WAIT 3-seed ablation training queue"
while ($true) {
    $missing = @($trainingCheckpoints | Where-Object { -not (Test-Path -LiteralPath $_) })
    if ($missing.Count -eq 0) { break }
    Write-QueueLog "WAIT training_remaining=$($missing.Count)"
    if ($DryRun) { break }
    Start-Sleep -Seconds $PollSeconds
}

if (-not $DryRun) {
    while ((Get-FreeGpuMemoryMB) -lt $RequiredFreeMemoryMB) {
        Write-QueueLog "WAIT gpu_free_mb=$(Get-FreeGpuMemoryMB) required=$RequiredFreeMemoryMB"
        Start-Sleep -Seconds $PollSeconds
    }
}

$jobs = @(
    [ordered]@{ RunName="lira_v2_dynamic_dualloss_seed2025"; Steps=3; MinFit=0.15; MinGain=0.001; RefineW=0.5; StableW=0.01 },
    [ordered]@{ RunName="lira_v2_ablation_t0_seed2025"; Steps=0; MinFit=0.15; MinGain=0.001; RefineW=0.0; StableW=0.01 },
    [ordered]@{ RunName="lira_v2_ablation_t1_seed2025"; Steps=1; MinFit=0.15; MinGain=0.001; RefineW=0.5; StableW=0.01 },
    [ordered]@{ RunName="lira_v2_ablation_no_stop_seed2025"; Steps=3; MinFit=0.0; MinGain=0.0; RefineW=0.5; StableW=0.01 },
    [ordered]@{ RunName="lira_v2_ablation_no_refined_loss_seed2025"; Steps=3; MinFit=0.15; MinGain=0.001; RefineW=0.0; StableW=0.01 },
    [ordered]@{ RunName="lira_v2_ablation_no_stability_seed2025"; Steps=3; MinFit=0.15; MinGain=0.001; RefineW=0.5; StableW=0.0 }
)
...
foreach ($job in $jobs) {
    ...
    $checkpointDir = Join-Path $checkpointRoot "strict_item_cold_balanced_thr1_seed_2025"
    ...
    if (-not (Test-Path -LiteralPath $checkpoint)) {
        throw "Missing validation-selected checkpoint: $checkpoint"
    }
    if (-not (Test-Path -LiteralPath $lockPath)) {
        throw "Missing locked training configuration: $lockPath"
    }
    ...
    if ([int]$locked.usim_steps -ne [int]$job.Steps -or
        ...
        throw "Locked training configuration does not match queued test replay: $($job.RunName)"
    }
    ...
    try {
        ...
        if ($LASTEXITCODE -ne 0) {
            throw "Test replay failed with exit code $LASTEXITCODE; see $stdout and $stderr"
        }
...
Write-QueueLog "ALL SEED2025 TEST TREND RUNS DONE"
