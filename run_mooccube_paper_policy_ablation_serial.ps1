param(
    ...
    [string]$OutputRootBase = "outputs\content_delta_pop5\course_policy_ablation_e60_3seed",
    [string]$CheckpointRootBase = "checkpoints\content_delta_pop5\course_policy_ablation_e60_3seed",
...
$QueueLog = Join-Path $OutputRootBaseAbs "course_policy_ablation_queue.log"
$PlanJson = Join-Path $OutputRootBaseAbs "course_policy_ablation_plan.json"
$PlanCsv = Join-Path $OutputRootBaseAbs "course_policy_ablation_plan.csv"
...
$Variants = @(
    (New-Variant `
        -Name "random_policy_rollout" `
        -Label "Random policy rollout" `
        -Rationale "Keep five-step simulator rollout and all course-aware signals, but choose each action uniformly at random without PPO optimization." `
        -Params @{ RolloutPolicy = "random"; UsimSteps = 5; PpoLossWeight = 0.0 }),
    (New-Variant `
        -Name "greedy_similarity_policy" `
        -Label "Greedy similarity policy" `
        -Rationale "Keep five-step simulator rollout and all course-aware signals, but choose the candidate most similar to the current item state without PPO optimization." `
        -Params @{ RolloutPolicy = "greedy_similarity"; UsimSteps = 5; PpoLossWeight = 0.0 }),
    (New-Variant `
        -Name "course_fit_policy" `
        -Label "Course-fit heuristic policy" `
        -Rationale "Keep five-step simulator rollout and all course-aware signals, but choose the candidate with the highest course-fit score without PPO optimization." `
        -Params @{ RolloutPolicy = "course_fit"; UsimSteps = 5; PpoLossWeight = 0.0 })
)
...
function Wait-ForPids {
    ...
    if ($unique.Count -lt 1) {
        Write-QueueLogLine "No active experiment PID to wait for; starting paper-main policy ablation queue."
        return
    }
    Write-QueueLogLine ("Waiting for experiment PIDs before paper-main policy ablation queue: {0}" -f (Format-ListValue $unique))
    while ($true) {
        $alive = @()
...
if ($DryRun) {
    Write-Host "MOOCCube paper-main policy ablation serial dry run"
...
Write-QueueLogLine "CONFIG MOOCCube paper-main policy ablation queue | seeds=$(Format-ListValue $SeedsToRun) | variants=$(Format-ListValue ($Variants.Name)) | baseline=$BaselineRoot | output=$OutputRootBase"
...
foreach ($variant in $Variants) {
    ...
    Write-QueueLogLine ("START {0} | label={1} | missing_seeds={2} | rollout_policy={3} | ppo_loss_weight={4} | usim_steps={5}" -f $variant.Name, $variant.Label, (Format-ListValue $missingSeeds), $runnerParams.RolloutPolicy, $runnerParams.PpoLossWeight, $runnerParams.UsimSteps)
...
Write-QueueLogLine "All requested MOOCCube paper-main policy ablations finished."
