...
# Fix 3: FixedSimpleAC moved here (SimpleAC dead code removed)
...
class PAM_RL_Pure_USIM(nn.Module):
    def __init__(self, config, content_emb):
        ...
        self.agent = FixedSimpleAC(config.emb_dim, time_dim=5)
            ...
            if n_total_neg > 0:
                ...
                if n_hard > 0:
                    if self.cfg.use_structured_hard_neg and self.item_hard_adj is not None:
                        ...
                        # Fix 4: false_neg_mask already applied to neg_logits above (redundant removed)
...
def main():
    ...
    # Fix 1: Incremental popularity — avoids future information leakage.
    # Initialized to zero; updated per-period from historical data only.
    item_incremental_pop = torch.zeros(cfg.n_items, dtype=torch.long)
    ...
    model.set_feedback_item_stats(item_incremental_pop)
    ...
    warmup_periods = int(os.environ.get("USIM_WARMUP_PERIODS", "3"))
    accumulated_dfs = []
    user_seen_items = {}
    ...
    for t in range(start_period, len(periods)):
        ...
        # Fix 2: Use previous period as validation set for early stopping (avoid test leakage)
        es_val_loader = None
        if t > 0 and t >= warmup_periods:
            es_val_ds = StreamDataset(periods[t - 1], llm_scores)
            es_val_loader = DataLoader(es_val_ds, batch_size=2048, shuffle=False, collate_fn=collate_fn)
        ...
        if not resume_this_period:
            ...
            # Fix 1 (cont): Update incremental popularity from current period's data
            if "i_idx" in p_df.columns and "popularity" in p_df.columns:
                period_pop = p_df.groupby("i_idx")["popularity"].max()
                for item_id, pop_value in period_pop.items():
                    idx = int(item_id)
                    if 0 <= idx < cfg.n_items:
                        item_incremental_pop[idx] = max(int(item_incremental_pop[idx].item()), int(pop_value))
                model.set_feedback_item_stats(item_incremental_pop)
    ...
    # Fix 5: Return final metrics for multi-seed aggregation
    final_results = {
        "full_cold": final_full_cold,
        "full_hot": final_full_hot,
        "sampled_cold": final_sampled_cold,
...
def main_multi_seed(seeds=None):
    """Fix 5: Multi-seed runner with mean±std reporting for statistical robustness."""
    if seeds is None:
        seed_str = os.environ.get("USIM_SEEDS", "")
        if seed_str.strip():
            seeds = [int(s.strip()) for s in seed_str.split(",") if s.strip()]
        else:
            seeds = [2025]
    if len(seeds) <= 1:
        setup_seed(seeds[0] if seeds else 2025)
        return main()
    all_results = []
    for seed in seeds:
        print(f"\n{'#' * 90}")
        print(f"# MULTI-SEED RUN: seed={seed} ({len(all_results)+1}/{len(seeds)})")
        print(f"{'#' * 90}")
        setup_seed(seed)
        result = main()
    ...
    print(f"\n{'=' * 90}")
    print(f"  MULTI-SEED SUMMARY ({len(all_results)} runs, seeds={seeds})")
    print(f"{'=' * 90}")
    for eval_key in ["full_cold", "full_hot", "sampled_cold", "sampled_hot"]:
        print(f"\n  [{eval_key}]")
        metric_keys = list(all_results[0].get(eval_key, {}).keys())
        for mk in metric_keys:
            vals = [r[eval_key][mk] for r in all_results if r.get(eval_key)]
            if not vals:
                continue
            mean_v = float(np.mean(vals))
            std_v = float(np.std(vals))
            print(f"    {mk:<10}: {mean_v:.4f} \u00b1 {std_v:.4f}  (runs: {[f'{v:.4f}' for v in vals]})")
    summary_rows = []
    for eval_key in ["full_cold", "full_hot"]:
        row = {"eval": eval_key, "n_runs": len(all_results), "seeds": str(seeds)}
        metric_keys = list(all_results[0].get(eval_key, {}).keys())
        for mk in metric_keys:
            vals = [r[eval_key][mk] for r in all_results if r.get(eval_key)]
    ...
    multi_seed_path = _feedback_output_path("multi_seed_summary.csv")
    pd.DataFrame(summary_rows).to_csv(multi_seed_path, index=False)
    print(f"\n>> Multi-seed summary saved to {multi_seed_path}")
    return all_results
