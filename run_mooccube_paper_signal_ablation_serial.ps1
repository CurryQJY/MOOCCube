param(
    ...
    [string]$OutputRootBase = "outputs\content_delta_pop5\course_signal_ablation_e60_3seed",
    [string]$CheckpointRootBase = "checkpoints\content_delta_pop5\course_signal_ablation_e60_3seed",
...
$Variants = @(
    (New-Variant `
        ...
        -Label "w/o Concept Match Signal" `
        -Rationale "Drop the positive course-concept match term while keeping other course signals active." `
        -Params @{ CourseConceptW = 0.0 }),
    (New-Variant `
        -Name "wo_prereq_signal" `
        -Label "w/o Prerequisite Signal" `
        -Rationale "Drop prerequisite reward penalty and neutralize the prerequisite safety gate." `
        -Params @{ CoursePrereqW = 0.0; CoursePrereqGate = 1.0 }),
    (New-Variant `
        -Name "wo_difficulty_signal" `
        -Label "w/o Difficulty Readiness Signal" `
        -Rationale "Drop the user-readiness versus item-difficulty penalty." `
        -Params @{ CourseDiffW = 0.0 }),
    (New-Variant `
        -Name "wo_redundancy_signal" `
        -Label "w/o Redundancy Control Signal" `
...
function Wait-ForPids {
    ...
    if ($unique.Count -lt 1) {
        Write-QueueLogLine "No active experiment PID to wait for; starting paper-main signal queue."
        return
    }
    Write-QueueLogLine ("Waiting for experiment PIDs before paper-main signal queue: {0}" -f (Format-ListValue $unique))
    while ($true) {
        $alive = @()
...
if ($DryRun) {
    Write-Host "MOOCCube paper-main course-signal ablation serial dry run"
...
Write-QueueLogLine "CONFIG MOOCCube paper-main course-signal ablation queue | seeds=$(Format-ListValue $SeedsToRun) | variants=$(Format-ListValue ($Variants.Name)) | baseline=$BaselineRoot | output=$OutputRootBase"
...
Write-QueueLogLine "All requested MOOCCube paper-main course-signal ablations finished."
