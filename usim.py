...
class Config:
    def __init__(self, n_users, n_items, content_dim=768):
        ...
        self.batch_size = int(os.environ.get('USIM_BATCH_SIZE', '2048'))        # 真实的大 Batch Size!
...
class PAM_RL_Pure_USIM(nn.Module):
    ...
    def get_item_vector(self, i_idx, llm_s, force_cold=False):
        # Keep a clean ID target for auxiliary alignment, even when the
        # input-side ID branch is masked by dropout/cold forcing.
...
def main():
    ...
    # ==============================
    print("\n" + "=" * 90)
    print(f"         FINAL REPORT: 采样评估 (1+{cfg.eval_n_neg}) vs 全库排名 (Pure RL-USIM + PAM Enhanced)")
    print("=" * 90)
    print(f"{'Metric':<10} | {'采样 Cold':<12} | {'采样 Hot':<12} | {'全库 Cold':<12} | {'全库 Hot':<12}")
    print("-" * 90)
