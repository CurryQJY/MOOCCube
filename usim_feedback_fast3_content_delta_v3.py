...
# [v3] Baseline-aligned split utility shared with BPR/LightGCN/MARec/CGRC.
from hin_data_common import static_split_df as _baseline_static_split_df
...
class BaseConfig:
    def __init__(self, n_users, n_items, content_dim=768):
        ...
        self.run_sampled_eval = os.environ.get("USIM_RUN_SAMPLED_EVAL", "1") == "1"
...
class PAM_RL_Pure_USIM(nn.Module):
    def __init__(self, config, content_emb):
        ...
        self.content_delta.weight.requires_grad = bool(getattr(config, "use_content_delta", False))
    ...
    def _apply_content_delta(self, content_e, i_idx, force_cold=False):
        ...
        if not getattr(self.cfg, "use_content_delta", False):
            ...
            return base_e
        delta = self.content_delta(i_idx)
    ...
    def content_delta_stats(self):
        if not getattr(self.cfg, "use_content_delta", False):
            return None
        norms = self.content_delta.weight.detach().norm(dim=1)
        max_norm = float(getattr(self.cfg, "content_delta_max_norm", 0.5))
        if max_norm > 0.0:
    ...
    def content_delta_regularization(self):
        zero = self.content_delta.weight.new_zeros(())
        if not getattr(self.cfg, "use_content_delta", False):
            return zero
        if not self.content_delta.weight.requires_grad:
            return zero
        max_norm = float(getattr(self.cfg, "content_delta_max_norm", 0.5))
        denom = max(max_norm, 1e-6) if max_norm > 0.0 else 1.0
        norm_ratio = self.content_delta.weight.norm(dim=1) / denom
        reg = zero
    ...
    def get_item_vector(self, i_idx, llm_s, force_cold=False, disable_id_dropout=False):
        ...
        content_base_e = self.content_proj(self.item_con_emb(i_idx))
        content_e = self._apply_content_delta(content_base_e, i_idx, force_cold=force_cold)
...
def evaluate_usim(model, loader, device, llm_scores, k_list=[5, 10, 20], n_neg=200,
                  ...
                  train_user_set=None, item_train_pop=None):
    """Evaluate USIM on the given loader.

    [v3 B2] When `train_user_set` and `item_train_pop` are provided, users
    not seen during training (cold users) get a popularity-based fallback
    score (log(1 + train_pop)) instead of the random-initialized user
    embedding. This makes v3 directly comparable with baselines that
    evaluate on the same user set, and gives cold users a reasonable
    non-random ranking signal.
    """
    model.eval()
    accum_metrics = {}
    total_samples = 0
    cold_user_samples = 0
    seen_tensor_cache = {}
    fallback_scores_full = None
    if train_user_set is not None and item_train_pop is not None:
        # Precompute the fallback ranking vector once: log1p(train popularity).
        fallback_scores_full = torch.as_tensor(
            item_train_pop, dtype=torch.float32, device=device
        ).clamp_min(0.0).log1p()
    with torch.no_grad():
        n_items = model.cfg.n_items
        all_item_idx = torch.arange(n_items, device=device)
        ...
        for batch, pop, llm in loader:
            ...
            # [v3 B2] Identify cold users (not seen during training) for fallback.
            cold_user_mask = None
            if fallback_scores_full is not None:
                cold_user_mask = torch.tensor(
                    [uid not in train_user_set for uid in user_ids],
                    dtype=torch.bool, device=device,
                )
                cold_user_samples += int(cold_user_mask.sum().item())
            if full_ranking:
                ...
                # [v3 B2] Cold-user fallback: replace cold-user score rows with
                # popularity-based ranking (log1p of train popularity).
                if cold_user_mask is not None and cold_user_mask.any():
                    fb = fallback_scores_full.unsqueeze(0).expand(int(cold_user_mask.sum().item()), -1).clone()
                    if user_seen_items:
                        cold_rows = cold_user_mask.nonzero(as_tuple=True)[0].tolist()
                        for local_idx, row in enumerate(cold_rows):
                            uid = user_ids[row]
                            seen_idx = seen_tensor_cache.get(uid)
                            if seen_idx is not None and seen_idx.numel() > 0:
                                fb[local_idx, seen_idx] = -1e9
                    # restore target item score so it is not -inf for cold users
                    fb_target = fallback_scores_full[i[cold_user_mask]]
                    fb[torch.arange(fb.size(0), device=device), i[cold_user_mask]] = fb_target
                    scores[cold_user_mask] = fb
                target_indices = i
            else:
                ...
                cand_vecs[:, 0, :] = pos_vec
                # [v3 fix] Shuffle candidate order to remove positional tie-bias toward the positive at idx=0
                perm = torch.argsort(
                    torch.rand(n_sel, cand_idx.size(1), device=device),
                    dim=1,
                )
                cand_idx = cand_idx.gather(1, perm)
                cand_vecs = cand_vecs.gather(
                    1, perm.unsqueeze(-1).expand(-1, -1, cand_vecs.size(-1))
                )
                ...
                # [v3 B2] Cold-user fallback for sampled ranking: rescore cand_idx by popularity.
                if cold_user_mask is not None and cold_user_mask.any():
                    fb_cand = fallback_scores_full[cand_idx[cold_user_mask]]
                    scores[cold_user_mask] = fb_cand
    ...
    if total_samples == 0:
        return None, 0
    if fallback_scores_full is not None and cold_user_samples > 0:
        print(
            f"  [v3 B2] cold-user fallback applied to {cold_user_samples}/{total_samples} "
            f"({100.0 * cold_user_samples / max(1, total_samples):.1f}%) evaluation samples."
        )
...
def _split_exact_warm_user(source_df, seed, train_ratio, val_ratio):
    """Global random split with per-user warm guarantee.

    Each user is forced to have at least one interaction in train (the
    "warm" guarantee), then the remaining interactions are shuffled
    globally and split into train/val/test by the target ratios.

    Caveats:
        - This is NOT a user-stratified 8:1:1 split. Some users may have
          all of their non-base interactions in test or val.
        - Single-interaction users only appear in train and never in
          val/test, so evaluation effectively excludes them.
        - Report this as "global random split with user-warm guarantee"
          to avoid confusion with baseline splits that are per-user.
    """
...
def _static_split_df(df):
    ...
    # [v3 fix] Drop duplicate (u, i) pairs before splitting to prevent
    # leakage where the same interaction appears across train/val/test.
    n_raw = len(df)
    work_df = (
        df.copy()
        .drop_duplicates(subset=["u_idx", "i_idx"], keep="first")
        .reset_index(drop=True)
    )
    n_after = len(work_df)
    if n_after < n_raw:
        print(
            f"[STATIC] Dropped {n_raw - n_after} duplicate (u, i) pairs before splitting "
            f"({n_raw} -> {n_after})."
        )
    work_df["_row_id"] = np.arange(len(work_df), dtype=np.int64)

    # [v3] Default: use baseline-aligned pure global random split so that v3 and
    # all baselines (BPR/LightGCN/MARec/CGRC) evaluate on the same user set.
    # Set USIM_STATIC_BASELINE_SPLIT=0 to fall back to the legacy warm-guarantee split.
    use_baseline_split = os.environ.get("USIM_STATIC_BASELINE_SPLIT", "1").strip() == "1"
    if use_baseline_split and not (split_mode in {"item_cold", "cold_item", "strict_item_cold"}):
        train_df_b, val_df_b, test_df_b = _baseline_static_split_df(
            work_df, seed=seed, train_ratio=train_ratio, val_ratio=val_ratio
        )
        train_users_b = set(int(u) for u in train_df_b["u_idx"].values)
        val_users_b = set(int(u) for u in val_df_b["u_idx"].values)
        test_users_b = set(int(u) for u in test_df_b["u_idx"].values)
        split_info = {
            "seed": seed,
            "split_mode": "baseline_global_random",
            "split_family": "baseline_global_random",
            "train_ratio": train_ratio,
            "val_ratio": val_ratio,
            "test_ratio": test_ratio,
            "train_rows": int(len(train_df_b)),
            "val_rows": int(len(val_df_b)),
            "test_rows": int(len(test_df_b)),
            "actual_train_ratio": float(len(train_df_b) / max(1, len(work_df))),
            "actual_val_ratio": float(len(val_df_b) / max(1, len(work_df))),
            "actual_test_ratio": float(len(test_df_b) / max(1, len(work_df))),
            "val_user_seen_ratio": float(len(val_users_b & train_users_b) / max(1, len(val_users_b))),
            "test_user_seen_ratio": float(len(test_users_b & train_users_b) / max(1, len(test_users_b))),
            "val_user_total": int(len(val_users_b)),
            "test_user_total": int(len(test_users_b)),
            "val_user_cold_count": int(len(val_users_b - train_users_b)),
            "test_user_cold_count": int(len(test_users_b - train_users_b)),
            "baseline_split": True,
        }
        print(
            f"[STATIC] Using baseline_global_random split (USIM_STATIC_BASELINE_SPLIT=1). "
            f"test_user_cold={split_info['test_user_cold_count']}/{split_info['test_user_total']} "
            f"({100.0 * split_info['test_user_cold_count'] / max(1, split_info['test_user_total']):.1f}%)"
        )
        return train_df_b, val_df_b, test_df_b, split_info

    strict_item_cold = split_mode in {"item_cold", "cold_item", "strict_item_cold"}
    coverage_moves = 0
    if strict_item_cold:
        ...
        rng_items.shuffle(eligible_items)
        ...
        val_cold_items = set(int(x) for x in eligible_items[:n_val_items])
        test_cold_items = set(int(x) for x in eligible_items[n_val_items:n_val_items + n_test_items])
...
def _make_fast3_optimizer(model, cfg):
    delta_param_id = id(model.content_delta.weight)
    base_params = [
        p for p in model.parameters()
        if p.requires_grad and id(p) != delta_param_id
    ]
    delta_params = [model.content_delta.weight] if model.content_delta.weight.requires_grad else []
    if delta_params:
...
def run_static_experiment(df, cfg, device, content_emb, llm_scores):
    ...
    # [v3 B2] Set of users seen during training. Used to detect cold users at
    # evaluation so we can apply popularity-based fallback for them.
    train_user_set = set(int(u) for u in train_df["u_idx"].values)
    ...
    for epoch in range(cfg.n_epochs):
        ...
        print(
            ...
            f"Time: {time.time() - epoch_start:.1f}s{delta_suffix}"
        )
        ...
        if do_early_stop:
            ...
            val_cold_metrics, val_cold_count = evaluate_usim(
                ...
                train_user_set=train_user_set, item_train_pop=item_train_pop,
            )
            ...
            cur_score = _metric_or_zero(val_cold_metrics, f"N@{cfg.early_stop_k}")
            print(
                f"  [STATIC-EARLYSTOP] Epoch {epoch + 1}: Full Cold N@{cfg.early_stop_k}={cur_score:.4f}, "
                f"Full Cold R@{cfg.early_stop_k}={_metric_or_zero(val_cold_metrics, f'R@{cfg.early_stop_k}'):.4f}, "
                f"Full Hot R@{cfg.early_stop_k}={_metric_or_zero(val_hot_metrics, f'R@{cfg.early_stop_k}'):.4f}"
            )
    ...
    if best_state is not None:
        ...
        if best_score <= -1e8:
            print(f"  [STATIC-EARLYSTOP] Restore best epoch={best_epoch} (validation not run)")
        else:
            print(f"  [STATIC-EARLYSTOP] Restore best epoch={best_epoch} (Full Cold N@{cfg.early_stop_k}={best_score:.4f})")
    ...
    print("\n" + "=" * 90)
    print("         FINAL REPORT (STATIC): sampled vs full ranking (RL-USIM FAST3 ContentDelta)")
    print("=" * 90)
    ...
    print(f"Full Samples: Cold={full_cold_count}, Hot={full_hot_count}")
    print("=" * 90)
...
def main():
    ...
    # [v3] Default to static protocol since v3 is the "static fixed" version of v1.
    # Set USIM_STATIC=0 to fall back to the original streaming protocol.
    if os.environ.get("USIM_STATIC", "1") == "1":
    ...
    delta_params = [
        model.content_delta.weight
    ] if model.content_delta.weight.requires_grad else []
    if delta_params:
    ...
    print(
        ...
        f"scale={cfg.content_delta_scale:.3f} | aux={cfg.content_delta_aux_mode} | "
