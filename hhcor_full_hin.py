...
class Config:
    def __init__(self, n_users: int, n_items: int, content_dim: int = 768):
        ...
        self.n_epochs = int(os.environ.get("HHCOR_FULL_EPOCHS", "3"))
        self.lr = 5e-4
        ...
        self.warmup_periods = int(os.environ.get("HHCOR_FULL_WARMUP", "2"))
        self.period_type = os.environ.get("HHCOR_PERIOD_TYPE", "M")
        self.use_cumulative = os.environ.get("HHCOR_USE_CUMULATIVE", "1") == "1"
...
def main():
    ...
    zeros_adj = torch.zeros(cfg.n_items, cfg.n_items, dtype=torch.float32)
    model.set_graph_buffers(concept_adj, zeros_adj, zeros_adj, item_cluster_ids)
    optimizer = torch.optim.Adam(model.parameters(), lr=cfg.lr)
    print(f"Model: HHCoR-stream | device={device}")
    ...
    print_final_report(
        ...
        title="HHCoR-Style Streaming HIN"
    )
    ...
    pd.DataFrame([out]).to_json("hhcor_full_result.json", orient="records", force_ascii=False)
    print("Saved: hhcor_full_result.json")
