...
$factorialRoot = "outputs\recppo_research_repair\simulator_factorial"
$outputRoot = Join-Path $factorialRoot "t0_training_course_fit"
$t5StaticRoot = "outputs\recovery_validation\main_table_51ea12fc_candidate"
$t5CourseFitRoot = "outputs\recppo_research_repair\test_course_fit_frozen\course_fit"
$t0StaticRoot = "outputs\recovery_validation\main_table_51ea12fc_core_ablation\wo_simulator"
$checkpointBefore = @{}
$completed = @()
...
try {
    ...
    foreach ($seed in $Seeds) {
        ...
        if ((Test-Path -LiteralPath $finalCsv) -and (Test-Path -LiteralPath $auditJson)) {
            Write-Host "SKIP completed simulator factorial arm: seed=$seed"
            $completed += $seed
            continue
        }

        Write-Host "===== SIMULATOR FACTORIAL seed=$seed train_steps=0 inference_steps=5 START $(Get-Date -Format o) ====="
        & $runner `
            ...
            -CourseTermNorm "none" -CourseSampleBeta 0.20 -TrainForceCold $true -UsimSteps 0 `
        ...
        if ($LASTEXITCODE -ne 0) {
            throw "Simulator factorial evaluation failed: seed=$seed exit=$LASTEXITCODE"
        }
        $completed += $seed
        Write-Host "===== SIMULATOR FACTORIAL seed=$seed train_steps=0 inference_steps=5 DONE $(Get-Date -Format o) ====="
    }
    ...
    foreach ($seed in $checkpointBefore.Keys) {
        ...
        if (-not $unchanged) {
            throw "Frozen T=0 checkpoint changed: seed=$seed"
        }
    }
    ...
    if ($completed.Count -gt 0) {
        ...
        & .\py.bat simulator_factorial_report.py `
            --t5-static-root $t5StaticRoot `
            --t5-course-fit-root $t5CourseFitRoot `
            --t0-static-root $t0StaticRoot `
            --t0-course-fit-root $outputRoot `
            --output-root $factorialRoot `
            --seeds $completed
        if ($LASTEXITCODE -ne 0) {
            throw "Simulator factorial report failed: exit=$LASTEXITCODE"
        }
    }
}
finally {
