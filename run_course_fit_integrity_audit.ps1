...
$baselineRoot = "outputs\recppo_research_repair\test_course_fit_frozen\course_fit"
$auditRoot = "outputs\recppo_research_repair\course_fit_integrity_audit"
$outputRoot = Join-Path $auditRoot "exclude_target_true"
$completed = @()
$checkpointBefore = @{}
...
try {
    ...
    $env:USIM_COURSE_MATCH_EXCLUDE_TARGET = "true"
    ...
    foreach ($seed in $Seeds) {
        ...
        $perItemCsv = Join-Path $armOutput "per_item_full_cold_usim_feedback_fast3_content_delta_static.csv"
        $auditJson = Join-Path $armOutput "actor_inference_audit.json"
        if ((Test-Path -LiteralPath $finalCsv) -and
            (Test-Path -LiteralPath $perItemCsv) -and
            (Test-Path -LiteralPath $auditJson)) {
            Write-Host "SKIP completed integrity arm: seed=$seed"
            $completed += $seed
            continue
        }

        Write-Host "===== COURSE-FIT INTEGRITY seed=$seed exclude_target=true START $(Get-Date -Format o) ====="
        & $runner `
        ...
        if ($LASTEXITCODE -ne 0) {
            throw "Course-fit integrity evaluation failed: seed=$seed exit=$LASTEXITCODE"
        }
        $completed += $seed
        Write-Host "===== COURSE-FIT INTEGRITY seed=$seed DONE $(Get-Date -Format o) ====="
    }
    ...
    foreach ($seed in $checkpointBefore.Keys) {
        ...
        if (-not $unchanged) {
            throw "Frozen checkpoint changed during integrity audit: seed=$seed"
        }
    }
    ...
    if ($completed.Count -gt 0) {
        ...
        $checkpointRows | ConvertTo-Json -Depth 4 | Set-Content `
            -LiteralPath (Join-Path $auditRoot "checkpoint_integrity.json") -Encoding utf8
        & .\py.bat course_fit_integrity_report.py `
            --baseline-root $baselineRoot `
            --audited-root $outputRoot `
            --output-root $auditRoot `
            --seeds $completed
        if ($LASTEXITCODE -ne 0) {
            throw "Course-fit integrity report failed: exit=$LASTEXITCODE"
        }
    }
}
finally {
