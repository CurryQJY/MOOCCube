...
class Config:
    def __init__(self, n_users: int, n_items: int):
        ...
        # EASE-style 评估（不归一化 user/item，保持 hist @ B 的原始语义）。默认 1 即开启 EASE 模式。
        self.ease_style_eval = os.environ.get("MAREC_EASE_STYLE_EVAL", "1").strip().lower() not in {
            "0",
            "false",
            "no",
        }
...
def _evaluate(
    ...
    ease_mode = bool(cfg.ease_style_eval)
    item_part = torch.tensor(item_vectors_np, dtype=torch.float32, device=device)
    if cfg.normalize_item_vectors and not ease_mode:
        item_part = torch.nn.functional.normalize(item_part, dim=1)
    # content_part 用于构造 user content profile；item bank 侧若 EASE 模式则不强行归一
    content_part_for_user = torch.nn.functional.normalize(content_vectors.to(device).float(), dim=1)
    content_part_for_item = content_part_for_user
    if beta <= 0.0:
        all_item_vectors = item_part
    elif beta >= 1.0:
        all_item_vectors = content_part_for_item
    else:
        all_item_vectors = torch.cat(
            [
                ((1.0 - beta) ** 0.5) * item_part,
                (beta ** 0.5) * content_part_for_item,
            ],
            dim=1,
        )
    ...
    def get_user_fn(batch):
        ...
        if beta <= 0.0:
            return hist
        profile = _build_user_content_profile(user_ids, train_seen, content_part_for_user, device)
        if not ease_mode:
    ...
    sample_cold, n_sc = evaluate_embedding_ranker(
        ...
        user_seen_items=test_seen, normalize_user=normalize_user,
    )
