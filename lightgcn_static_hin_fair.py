...
class Config:
    def __init__(self, n_users: int, n_items: int, content_dim: int = 768):
        ...
        self.ckpt_dir = os.environ.get("LIGHTGCN_CKPT_DIR", default_ckpt_dir).strip()
        self.save_ckpt = _env_flag("LIGHTGCN_SAVE_CKPT", _env_flag("BASELINE_SAVE_CKPT", bool(self.ckpt_dir)))
        self.auto_resume = _env_flag("LIGHTGCN_AUTO_RESUME", _env_flag("BASELINE_AUTO_RESUME", bool(self.ckpt_dir)))
        self.force_fresh = _env_flag("LIGHTGCN_FORCE_FRESH", _env_flag("BASELINE_FORCE_FRESH", False))
        self.save_opt_state = _env_flag("LIGHTGCN_SAVE_OPT_STATE", _env_flag("BASELINE_SAVE_OPT_STATE", True))
...
def main():
    ...
    with torch.no_grad():
        ...
        full_cold_item_macro, n_fc_item_macro = evaluate_embedding_ranker(
            ...
            export_item_metrics_path=static_result_path("per_item_full_cold_lightgcn_static.csv"),
        )
        full_hot_item_macro, n_fh_item_macro = evaluate_embedding_ranker(
            test_loader, device, cfg.n_items, cfg.cold_threshold, get_user_fn, all_i,
            k_list=k_list, n_neg=cfg.eval_n_neg, eval_type="hot", full_ranking=True,
            user_seen_items=test_seen, average_mode="item_macro",
            export_item_metrics_path=static_result_path("per_item_full_hot_lightgcn_static.csv"),
        )
    ...
    out = {
        ...
        "per_item_full_cold_path": static_result_path("per_item_full_cold_lightgcn_static.csv"),
        "per_item_full_hot_path": static_result_path("per_item_full_hot_lightgcn_static.csv"),
    }
