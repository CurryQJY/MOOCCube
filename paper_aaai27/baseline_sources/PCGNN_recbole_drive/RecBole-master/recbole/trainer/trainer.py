...
class Trainer(AbstractTrainer):
    ...
    def evaluate(self, eval_data, load_best_model=True, model_file=None, show_progress=False):
        ...
        self.model.eval()
        # 此时，eval data里面并不是理解的每个用户只有一条，例如前五条都是用户1的记录
        # print(eval_data.dataset)
        # print(len(eval_data))
        # print(eval_data.dataset[0:5]['user_id'])
        # print(eval_data.dataset[500:505]['user_id'])
        # exit()
        ...
        for batch_idx, batched_data in iter_data:
            if eval_data.dl_type == DataLoaderType.FULL:
                ...
                # interaction里的userid是按顺序来的1，2，3......
                # print(interaction[0:10]['user_id'])
                # exit()
            else:
            ...
            # collect：collect the all used evaluators' intermediate result of one batch.
...
class kg_modelTrainer(Trainer):

    def __init__(self, config, model):
        super(kg_modelTrainer, self).__init__(config, model)
    ...
    #         kg_losses = loss_func(interaction)
    #
    #         kg_loss = kg_losses
    #         kg_total_loss = kg_losses.item() if kg_total_loss is None else kg_total_loss + kg_losses.item()
    #         self._check_nan(kg_loss)
    #         kg_loss.backward()
    #         if self.clip_grad_norm:
    #             clip_grad_norm_(self.model.parameters(), **self.clip_grad_norm)
    #         self.optimizer.step()
    ...
    #         rs_losses = loss_func(interaction)
    #         rs_loss = rs_losses
    #         rs_total_loss = rs_losses.item() if rs_total_loss is None else rs_total_loss + rs_losses.item()
    #         self._check_nan(rs_loss)
    #         rs_loss.backward()
    #         if self.clip_grad_norm:
    #             clip_grad_norm_(self.model.parameters(), **self.clip_grad_norm)
    #         self.optimizer.step()
    #     # 可以返回两个loss，tuple会自动sum
    #
    ...
    #     # 改变了mode后。这里可能有问题，会整整跑一个epoch之后才会换，而不是论文中的交替训练
    #     # __iter__中，会根据其返回不同数据，只有为RSKG时才会返回self，只执行一次
    #     # setmode中说：would affect the result of _next_batch_data()
    #     rs_total_loss = super()._train_epoch(train_data, epoch_idx,loss_func=self.model.calculate_rs_loss,show_progress=show_progress)
    #     # train kg
    ...
    def _train_epoch(self, train_data, epoch_idx, loss_func=None, show_progress=False):
        ...
        for batch_idx, interaction in iter_data:
            ...
            # kg loss
            loss_func = self.model.calculate_kg_loss
            kg_losses = loss_func(interaction)+self.model.calculate_rs_loss(interaction)
    ...
    #     if epoch_idx == 0:
    #
    #         predict_total_loss = super()._train_epoch(
    #             train_data, epoch_idx, loss_func=self.model.calculate_cat_loss, show_progress=show_progress
    #         )
    #     iter_data = (
    #         tqdm(
    #             enumerate(train_data),
    ...
    #
    #         kg_loss = kg_losses
    #         total_loss = kg_losses.item() if total_loss is None else total_loss + kg_losses.item()
    #         self._check_nan(kg_loss)
    #         kg_loss.backward()
    #         if self.clip_grad_norm:
    #             clip_grad_norm_(self.model.parameters(), **self.clip_grad_norm)
