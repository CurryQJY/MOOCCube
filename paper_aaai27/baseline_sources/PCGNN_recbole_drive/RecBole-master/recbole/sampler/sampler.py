...
class AbstractSampler(object):
    ...
    def random_num(self, num):
        ...
        """
        # 类似batch的处理，如果要取的小于list，直接取了，如果大于，要取好几次
        value_id = []
    ...
    def sample_by_key_ids(self, key_ids, num):
        ...
        if (key_ids == key_ids[0]).all():
            ...
            # 函数将函数沿着X轴扩大num倍。如果扩大倍数只有一个，默认为X轴
            key_ids = np.tile(key_ids, num)
            while len(check_list) > 0:
                # 相当于第一次默认check list元素全部重复，全重抽
                value_ids[check_list] = self.random_num(len(check_list))
                # 减少value ids中出现过的id
                # self.used_ids[key_ids[check_list]]：used ids以key id为索引找到其交互过的item
                # 因为zip了,所以以循环的形式,第一个循环为check_list第一个值，并判断v是否在used中，不在则保留在check_list中
                check_list = np.array([
...
class Sampler(AbstractSampler):
    ...
    def get_used_ids(self):
        ...
        Returns:
            ...
            Key is phase, and value is a numpy.ndarray which index is user_id, and element is a set of item_ids.
        """
        # key是phase，value是数组，数组中的index是uid，元素是它交互过的iid集合（set）
        used_item_id = dict()
        last = [set() for _ in range(self.n_users)]
        for phase, dataset in zip(self.phases, self.datasets):
            # cur：[set() set() set() set() set() set() set() set() set() set()]这样的
            cur = np.array([set(s) for s in last])
            # 向cur中添加交互过的iid，uid为索引。一个索引里可能有多个item id
...
class RepeatableSampler(AbstractSampler):
    ...
    # 以实际代码来看，RepeatableSampler并不是它所说的只能对在所有阶段都没出现过的item进行采样，而是可以对所有item进行采样，即使与其交互过的
