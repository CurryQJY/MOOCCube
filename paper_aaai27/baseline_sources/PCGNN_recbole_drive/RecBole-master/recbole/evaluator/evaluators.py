...
class TopKEvaluator(GroupedEvaluator):
    ...
    def collect(self, interaction, scores_tensor):
       ...
       """
        ...
        # 修改
        shape_matrix = torch.full((len(user_len_list), 1), scores_matrix.shape[1], device=scores_matrix.device,dtype=torch.long)
        # get topk
    ...
    def evaluate(self, batch_matrix_list, eval_data):
        ...
        """
        # evaluate就不是在一个batch内了，而是读取所有的数据之后再进行evaluate
        #  Number of positive item for each user in a training/evaluating epoch.
        pos_len_list = eval_data.get_pos_len_list() #结果是[1 1 1 ... 1 1 1]
        # 大小为（总用户数，topk+1），加的1是额外存了item的数量
        ...
        # li = np.load('5_10.npy')
        # li = np.load('11_20.npy')
        # li = np.load('21_1000.npy')
        # topk_idx = batch_result[li-1, :-1]
        # pos_len_list = pos_len_list[li-1]
        # shapes = shapes[li-1]
        # print(topk_idx[0:2])
        #
        #
