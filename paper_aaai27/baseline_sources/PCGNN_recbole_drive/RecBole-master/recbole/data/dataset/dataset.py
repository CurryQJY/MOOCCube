...
class Dataset(object):
    ...
    def __init__(self, config):
        ...
        # self.inter_feat在这里还是dataframe类型
    ...
    def _data_processing(self):
        ...
        # 似乎只是会过滤重复的，根据user和item过滤inter里没出现的，而不会根据inter过滤item没出现过的
        if self.benchmark_filename_list is None:
            self._data_filtering()
            # print('*'*100)会执行

        self._remap_ID_all()
        # print(self.field2id_token['user_id'][:10])
    ...
    def _load_data(self, token, dataset_path):
        ...
        # 如果load_col没有user，item，那下面两个都是none
    ...
    def _user_item_feat_preparation(self):
        ...
        if self.user_feat is not None:
            # 相当于建了只有一列的dataframe。userid为列名，有user_num行.这部我觉得就是用来排序，从0开始,而0会nan
            # 很久后又看了这个方法：user_feat可能现在就没有id的信息了，只有01234的index，因为记不得了，所以是猜测
    ...
    def _remap_ID_all(self):
        ...
        for field_set in fields_in_same_space:
            # print(field_set)
            # 返回需要remap的信息（feat（dataframe），field和type）
    ...
    def _concat_remaped_tokens(self, remap_list):
        ...
        for feat, field, ftype in remap_list:
            # print(feat)
            # print(feat[field])
            # print(feat[field].values)
            # print(field)
            # feat[field]就使得只包含了userid，itemid之类
            # （第一轮是userid remap，第二轮是itemid，不在此方法中，在外层）
            if ftype == FeatureType.TOKEN:
    ...
    def _remap(self, remap_list):
        ...
        # print(new_ids_list)
        # new_ids_list和mp分别记录idx和原来的id，token_id则是对应关系,且正好0对应pad
        # 而且new_ids_list有两部分，分别记录inter，user的userid。第二轮则是记录inter，item的itemid
    ...
    def num(self, field):
        ...
        # 应该就是对每个field不重复的计数
        # 说明第一行会被pad掉
        # print('123', self.field2id_token[field])
        if field not in self.field2type:
            raise ValueError(f'Field [{field}] not defined in dataset.')
    ...
    def join(self, df):
        ...
        """
        # df看似是dataframe，其实是Interaction类型
        # df[self.iid_field]得到索引
        # 而且由于df[self.iid_field]可能是batchsize*maxLen,索引是多维的torch
        # ，然后interaction对象通过索引取的是每个键里对应索引位置的值
