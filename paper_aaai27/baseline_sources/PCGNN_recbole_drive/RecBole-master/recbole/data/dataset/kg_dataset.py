...
class KnowledgeBasedDataset(Dataset):
    ...
    def _create_ckg_sparse_matrix(self, form='coo', show_relation=False):
        ...
        # hids，tids实体list，加上user_num相当于索引都往后挪了，同理iid也是

        # 应该是kg_feat[self.head_entity_field]不包含user的
        # 实际上，hids，tids包含了item和其他一些实体，因此只要加user_num,因为这样之后相同索引的位置是和item_id一样的
        ...
        # 猜测因为非序列的dataset中，都是(u,i)形式，因此ui关系个数就是u的长度（i也一样）
        # 这样也保证了src与tgt能对应上，即对应的u与i
        ui_rel_num = len(uids)
        ui_rel_id = self.relation_num - 1
        # self.relation_field即relation_id,类似uid_field就是userid这个字符串
        assert self.field2id_token[self.relation_field][ui_rel_id] == '[UI-Relation]'
        # 横向拼，相当于uid对应iid，iid对应uid，hid对应tid。然后src与tgt的每一对都算是h,r,t，包括u与i之间的关系
        ...
        if not show_relation:
            ...
            # 构造2 * ui_rel_num大小的数组，用ui_rel_id(relation最后一个id)填充
            # 目的是和 (src, tgt)对应上,即uid,iid与iid,uid都是ui关系,而hids,tids是kg_rel关系
