...
class SequentialDataset(Dataset):
    ...
    def prepare_data_augmentation(self):
        ...
        seq_start = 0
        # 因为之前已经按uid，time排好序了，如果出现就是第一个用户，第一个它交互的item。如果换个用户，就重新计数
        # i应该记录的是交互数，控制结束位置，而且是要预测的item。seq_start控制开始位置
        # inter_feat[self.uid_field]我猜应该是关于uid那一列的信息，uid可能重复出现，出现次数和他的交互数一样。（没看dataset.py）
        ...
        for i, uid in enumerate(self.inter_feat[self.uid_field].numpy()):
            ...
            if last_uid != uid:
                ...
                # 因为max_item_list_len似乎默认50，而数据很有可能超过这个长度，因此seq_start要跟着加（感觉相当于小于50的，就记录1到50.而大于50的，就只记录k到50+k）
                # ****************************************************
                # 即rnn长度其实有上限的，最多就50层，如果多了就像时间序列那样，以50个预测1个。
                # ****************************************************
                ...
                uid_list.append(uid)
                # 这里应该只是往item_list_index里加slice元素，没拆分arr，等需要的时候再arr[slice]
                # uid_list和item_list_index一一对应，但一个uid可能对应好几个item(对应一个item序列)
