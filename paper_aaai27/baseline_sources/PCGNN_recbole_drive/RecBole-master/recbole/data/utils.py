...
def create_dataset(config):
    ...
    # 如果dataset名字直接叫modelDataset，那就会直接用这个dataset
...
def data_preparation(config, dataset, save=False):
    ...
    # 这里的built_datasets是个list，而且里面元素基本一样，就mask不一样，具体可看sequential_dataset的leave_one_out方法
    # 因此在这里对train_dataset进行裁剪20%数据没有用
    ...
    if train_neg_sample_args['strategy'] != 'none':
        ...
        # 这样会使模型中既有general，又有kg的sampler，但不适合我们的，我们需要要序列的
        if model_type != ModelType.SEQUENTIAL:
            sampler = Sampler(phases, built_datasets, train_neg_sample_args['distribution'])
        else:
            # 与一般sampler不同的是，它只能对在所有阶段都没出现过的item进行采样
    ...
    # 在这里加了
    if config['model'] == 'kg_model':
        kg_sampler = KGSampler(dataset, 'uniform')
        train_kwargs['kg_sampler'] = kg_sampler
...
def get_data_loader(name, config, neg_sample_args):
    ...
    register_table = {
        ...
        'kg_model': _get_kg_model_data_loader,
    ...
    # 如果在model_type_table，即正常的那些模型，就会返回对应名字.然后根据neg_sample_strategy选择底下对应的fulldataloader还是neg
    if model_type in model_type_table and neg_sample_strategy in neg_sample_strategy_table:
        ...
        # 一般这种模型可以是full，但是都是training_neg_sample_num不为0，这种训练阶段就会变成by，测试和评估就变成full
...
def _get_kg_model_data_loader(name, config, neg_sample_args):

    neg_sample_strategy = neg_sample_args['strategy']

    # 因为是pointwise
    if neg_sample_strategy == 'none':
        return kg_modelDataLoader
