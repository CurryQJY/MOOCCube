...
class SequentialDataLoader(AbstractDataLoader):
    ...
    def __init__(self, config, dataset, batch_size=1, dl_format=InputType.POINTWISE, shuffle=False):
        ...
        for field in dataset.inter_feat:
            if field != self.uid_field:
                ...
                # field2type将feature name变成它的类型（应该比如说性别中的男女），field2id_token则是将feature（id）映射成idx
    ...
    def _shuffle(self):
        ...
        #             such as neg-sampling and data-augmentation.意思是时间有否为真，是的话就按时间序列来处理（也可能序列模型的realtime都是true）
        if self.real_time:
...
class SequentialFullDataLoader(NegSampleMixin, SequentialDataLoader):
    ...
    def __init__(
        ...
        # srgnn只有验证和测试的时候会调用该方法，训练的时候还是用SequentialDataLoader
        # sampler:<recbole.sampler.sampler.RepeatableSampler object at 0x000001F450438CC8>
