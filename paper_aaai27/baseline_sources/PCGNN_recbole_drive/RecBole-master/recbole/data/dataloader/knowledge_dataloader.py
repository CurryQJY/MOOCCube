...
################################################
"""
from recbole.data.dataloader import SequentialNegSampleDataLoader
from recbole.data.dataloader import SequentialDataLoader
from recbole.data.dataloader import NegSampleMixin
from recbole.data.dataloader import SequentialFullDataLoader
...
class KnowledgeBasedDataLoader(AbstractDataLoader):
    ...
    def set_mode(self, state):
        ...
        self.state = state
class kg_modelDataLoader(AbstractDataLoader):
    ...
    def __init__(
        ...
        # using sampler
        self.seq_dataloader = SequentialDataLoader(
            config=config,
            dataset=dataset,
            batch_size=batch_size,
            dl_format=dl_format,
        ...
        # self.seq_dataloader = SequentialNegSampleDataLoader(
        #     config=config,
        #     dataset=dataset,
        #     batch_size=batch_size,
        #     dl_format=InputType.PAIRTWISE,
        #     shuffle=shuffle
        # )
    ...
    def __iter__(self):
        ...
        if self.state == KGDataLoaderState.KG:
            ...
            return self.seq_dataloader.__iter__()
        elif self.state == KGDataLoaderState.RSKG:
            self.kg_dataloader.__iter__()
            self.seq_dataloader.__iter__()
            return self
    ...
    def __next__(self):
        if self.seq_dataloader.pr >= self.seq_dataloader.pr_end:
            self.seq_dataloader.pr = 0
            self.kg_dataloader.pr = 0
            raise StopIteration()
        return self._next_batch_data()
    ...
    def __len__(self):
        if self.state == KGDataLoaderState.KG:
            ...
            return len(self.seq_dataloader)
    ...
    def pr_end(self):
        ...
        if self.state == KGDataLoaderState.KG:
            ...
            return self.seq_dataloader.pr_end
    ...
    def _next_batch_data(self):
        try:
            ...
            kg_data = self.kg_dataloader.__next__()
        rec_data = self.seq_dataloader.__next__()
        rec_data.update(kg_data)
        return rec_data
