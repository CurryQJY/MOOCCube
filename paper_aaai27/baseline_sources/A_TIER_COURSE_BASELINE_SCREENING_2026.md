...
| Recent course-specific source | MSEC-Rec | IP&M 2026 | Official code: <https://github.com/mmx124/MSEC-Rec>; directly consumes user-course, user-video, course-video, course-knowledge, and video-concept relations | The audited HAN outputs user representations only while candidates remain course-ID embeddings; strict cold support would require a new course encoder. The configured random walks are also infeasible at the current user scale. | Source-valid related baseline, but strict item-cold NO-GO without method redesign |
| Newest source-first course lead | IDRMI | ESWA 2025 | Paper-indexed PyTorch source: <https://github.com/miaomiao924/IDRMI>; model modules are present but public data are omitted | Repair the released empty NGCF adjacency, remove hard-coded sizes/CUDA, build all heuristic factors from train-only history, and replace label classification with strict full-catalog course-macro evaluation | Run only a small source-repair feasibility gate; stop if source fidelity requires a method redesign |
| Lower-risk course fallback | KGAN | ESWA 2023 | Official code: <https://github.com/StZHY/KGAN>; released MOOCCube course data | Replace hard-coded random split/evaluator and port TensorFlow 1.10 implementation to a supported stack | Prioritize after IDRMI's gate under the user's source-first rule |
