    ...
    with torch.no_grad():
        for batch, pop in loader:
            ...
            if user_seen_items:
                ...
                # keep target valid if it was in seen cache by construction issue
                scores[rows, t_cols] = pos_scores
...
def main():
    ...
    dataloaders = [DataLoader(StreamDataset(p), batch_size=2048, shuffle=True, collate_fn=collate_fn) for p in periods]
    ...
    for t, loader in enumerate(dataloaders):
        ...
        # --- Phase 1: Eval ---
        # Use history graph BEFORE adding current period edges (no look-ahead).
        if t >= WARMUP_PERIODS and len(global_u_idx) > 0:
            eval_adj, eval_svd_u, eval_svd_s, eval_svd_v = build_sparse_adj_and_svd(
                global_u_idx, global_i_idx, cfg.n_users, cfg.n_items, q=5, device=device
            )
            model.eval()
            with torch.no_grad():
                all_z_u, all_z_i = model(eval_adj, eval_svd_u, eval_svd_s, eval_svd_v, u_idx=None)
                all_z_u = F.normalize(all_z_u, dim=1)
                all_z_i = F.normalize(all_z_i, dim=1)
                
            c_m_f, n_c_f, h_m_f, n_h_f = evaluate_dual_lightgcl(
                loader, all_z_u.cpu(), all_z_i.cpu(), device, k_list, cfg.cold_threshold, user_seen_items=user_seen_items
            )
            c_m_s, n_c_s, h_m_s, n_h_s = evaluate_sampled_lightgcl(
                loader, all_z_u.cpu(), all_z_i.cpu(), device, k_list, cfg.cold_threshold, cfg.n_items, user_seen_items=user_seen_items
            )
        ...
        # Add current period edges after evaluation.
        global_u_idx.extend(cur_u)
        global_i_idx.extend(cur_i)

        # Build train graph on history + current period.
        adj_norm, svd_u, svd_s, svd_v = build_sparse_adj_and_svd(
            global_u_idx, global_i_idx, cfg.n_users, cfg.n_items, q=5, device=device
        )

        # --- Phase 2: Train (LightGCL + InfoNCE) ---
        model.train()
        total_main_loss = 0
        total_cl_loss = 0
        steps = 0
        ...
        for batch, pop in loader:
            ...
            # 1. InfoNCE Recommender Loss (Simulates modern Large Batch)
            ...
            # 2. SVD Contrastive Loss (LightGCL unique)
            # cl(E_u, G_u) + cl(E_i, G_i) -> 使得图结构特征紧贴低频奇异值成分，抗噪。
        ...
        print(f"  [TRAIN] InfoNCE: {total_main_loss / steps:.4f} | SVD CL: {total_cl_loss / steps:.4f}")

        # Update seen interactions after this period (for next period's eval masking).
    ...
    print("\n" + "=" * 90)
    print("         FINAL RESULT: Sampled (1+999) vs Full Ranking (LightGCL ICLR'23)")
    print("=" * 90)
    ...
    out = {"model": "LightGCL", "protocol": "stream"}
    for k in metrics_keys:
    ...
    pd.DataFrame([out]).to_json("lightgcl_full_result.json", orient="records", force_ascii=False)
    print("Saved: lightgcl_full_result.json")
