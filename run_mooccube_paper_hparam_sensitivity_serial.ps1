param(
    ...
    [string]$OutputRootBase = "outputs\content_delta_pop5\course_hparam_sensitivity_e60_3seed",
    [string]$CheckpointRootBase = "checkpoints\content_delta_pop5\course_hparam_sensitivity_e60_3seed",
...
$QueueLog = Join-Path $OutputRootBaseAbs "course_hparam_sensitivity_queue.log"
$PlanJson = Join-Path $OutputRootBaseAbs "course_hparam_sensitivity_plan.json"
$PlanCsv = Join-Path $OutputRootBaseAbs "course_hparam_sensitivity_plan.csv"
...
$Variants = @(
    (New-Variant `
        -Name "sample_beta_0p10" `
        -Label "Course sample beta 0.10" `
        -Rationale "Lower the course-aware candidate sampling strength from the paper-main value 0.20 to 0.10." `
        -Params @{ CourseSampleBeta = 0.10 }),
    (New-Variant `
        -Name "sample_beta_0p30" `
        -Label "Course sample beta 0.30" `
        -Rationale "Raise the course-aware candidate sampling strength from the paper-main value 0.20 to 0.30." `
        -Params @{ CourseSampleBeta = 0.30 }),
    (New-Variant `
        -Name "reward_scale_0p5" `
        -Label "Course reward weights x0.5" `
        -Rationale "Halve all course reward-term weights while keeping candidate sampling and auxiliary loss fixed." `
        -Params @{
            CoursePrereqW = 0.04
            CourseConceptW = 0.02
            CourseDiffW = 0.015
    ...
    (New-Variant `
        -Name "reward_scale_1p5" `
        -Label "Course reward weights x1.5" `
        -Rationale "Increase all course reward-term weights by 50 percent while keeping candidate sampling and auxiliary loss fixed." `
        -Params @{
            CoursePrereqW = 0.12
            CourseConceptW = 0.06
            CourseDiffW = 0.045
    ...
    (New-Variant `
        -Name "term_norm_batch" `
        -Label "Batch-normalized course terms" `
        -Rationale "Switch course-term normalization from none to batch to test reward-scale stability." `
        -Params @{ CourseTermNorm = "batch" }),
    (New-Variant `
        -Name "prereq_gate_0p35" `
        -Label "Prerequisite gate 0.35" `
        -Rationale "Relax the prerequisite safety gate from 0.20 to 0.35 without removing prerequisite reward shaping." `
        -Params @{ CoursePrereqGate = 0.35 })
)
...
function Wait-ForPids {
    ...
    if ($unique.Count -lt 1) {
        Write-QueueLogLine "No active experiment PID to wait for; starting paper-main hyperparam queue."
        return
    }
    Write-QueueLogLine ("Waiting for experiment PIDs before paper-main hyperparam queue: {0}" -f (Format-ListValue $unique))
    while ($true) {
        $alive = @()
...
function Write-PlanFiles {
    ...
    foreach ($variant in $Variants) {
        ...
        $planRows += [pscustomobject]@{
            ...
            use_sage_lite = $params.UseSageLite
            use_sage_aux_loss = $params.UseSageAuxLoss
...
if ($DryRun) {
    Write-Host "MOOCCube paper-main hyperparam sensitivity serial dry run"
...
Write-QueueLogLine "CONFIG MOOCCube paper-main hyperparam sensitivity queue | seeds=$(Format-ListValue $SeedsToRun) | variants=$(Format-ListValue ($Variants.Name)) | baseline=$BaselineRoot | output=$OutputRootBase"
...
Write-QueueLogLine "All requested MOOCCube paper-main hyperparam sensitivity runs finished."
