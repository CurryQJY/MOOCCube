...
- Reduce both tilted representations from width `63` to width `54` so they can move outward without touching the dashed box.
- Place the representation centers at `flow_cx - 85` and `flow_cx + 85`.
...
- Assert the two representation centers and labels are at `flow_cx - 85` and `flow_cx + 85`.
- Assert the tilted-vector calls use width `54` and remain equidistant from `flow_cx`.
