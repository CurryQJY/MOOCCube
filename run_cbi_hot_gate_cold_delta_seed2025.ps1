...
$outputRoot = "outputs\cbi_hot_gate_cold_delta_seed2025"
$checkpointRoot = "checkpoints\cbi_hot_gate_cold_delta_seed2025"
$logRoot = "background_logs\cbi_hot_gate_cold_delta_seed2025"
$manifestPath = Join-Path $outputRoot "run_manifest.json"
$logPath = Join-Path $logRoot "training.log"

$runnerParams = [ordered]@{
    PythonRunner=".\py.bat"; ScriptPath="cbi_hot_gate_cold_delta_seed2025.py"
    ...
    ContentDeltaMode="embedding"; ContentDeltaMaxNorm=0.5; ContentDeltaScale=1.0
    ContentDeltaLrMult=1.0; ContentDeltaL2W=0.0; ContentDeltaCapW=0.0
    ContentDeltaAuxMode="base"; AuxWeight=0.3; AuxHotOnly=$false
    UsePseudoColdTrain=$false; PseudoColdMode="batch_random"; PseudoColdRatio=0.30; PseudoColdMinPop=5
...
$lockedConfig = [ordered]@{
    experiment="cbi_hot_gate_cold_delta_seed2025"
    method="hot_gate_native_scale_cold_only_content_delta"
    hot_only_gate=$true; cold_bypasses_gate=$true; normalize_hot_fused_before_simulation=$false
    content_delta_cold_only=$true; target_anchor="initial_cbi"
    selection="overall_floor_then_maximize_cold"; snapshot_epochs=@(1,2,3,4,5,6,7,8)
...
$sourceFiles=@("run_cbi_hot_gate_cold_delta_seed2025.ps1","cbi_hot_gate_cold_delta_seed2025.py","cbi_hot_gate_audit_seed2025.py","cbi_anchor_sim.py","cbi_trust_sim.py","evaluate_cbi_all_refined_seed2025.py","run_usim_feedback_fast3_content_delta_static.ps1")
...
$manifest=[ordered]@{schema_version=1;experiment="cbi_hot_gate_cold_delta_seed2025";status="running";started_at_utc=(Get-Date).ToUniversalTime().ToString("o");completed_at_utc=$null;elapsed_seconds=$null;exit_code=$null;error=$null;repo=$repoPath;git_commit=(git rev-parse HEAD).Trim();git_dirty=@((git status --porcelain)).Count -gt 0;locked_config=$lockedConfig;source_sha256=Get-HashMap $sourceFiles;protected_files_before=$before;protected_files_after=$null}
...
if($null -ne $runError){throw $runError};Write-Host "Hot-gate cold-delta seed-2025 experiment completed."
