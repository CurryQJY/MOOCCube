...
$script = Join-Path $repo "run_fsgnn_coldrec_3datasets_3seed_serial.ps1"

if (-not (Test-Path -LiteralPath $script)) {
    throw "Missing ColdRec FS-GNN serial runner: $script"
}

$tmpRoot = Join-Path ([System.IO.Path]::GetTempPath()) ("fsgnn_coldrec_serial_" + [System.Guid]::NewGuid().ToString("N"))
...
Write-Host "test_fsgnn_coldrec_3dataset_serial.ps1 passed"
