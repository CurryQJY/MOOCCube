param(
    ...
    [int[]]$SeedsToRun = @(2026, 2027),
    [string]$SeedList = "",
    [string]$VariantList = "",
    [switch]$AllWideVariants,
    [int[]]$WaitPid = @(),
    [switch]$NoAutoWait,
    [int]$PollSeconds = 300,
...
$QueueLog = Join-Path $OutputRootBaseAbs "course_hparam_wide_multiseed_queue.log"
$PlanJson = Join-Path $OutputRootBaseAbs "course_hparam_wide_multiseed_plan.json"
$PlanCsv = Join-Path $OutputRootBaseAbs "course_hparam_wide_multiseed_plan.csv"
...
$Variants = @(
    ...
    (New-Variant -Name "reward_scale_0p75" -Family "reward_scale" -Label "Course reward weights x0.75" -Rationale "Supplement the seed2025 reward-scale candidate with the remaining paper seeds." -Params @{ CoursePrereqW = 0.06; CourseConceptW = 0.03; CourseDiffW = 0.0225; CourseRedundantW = 0.015 }),
    (New-Variant -Name "reward_scale_1p25" -Family "reward_scale" -Label "Course reward weights x1.25" -Rationale "Supplement the seed2025 reward-scale candidate with the remaining paper seeds." -Params @{ CoursePrereqW = 0.10; CourseConceptW = 0.05; CourseDiffW = 0.0375; CourseRedundantW = 0.025 }),
    ...
    (New-Variant -Name "prereq_gate_0p70" -Family "prereq_gate" -Label "Prerequisite gate 0.70" -Rationale "Supplement the strongest seed2025 prerequisite-gate candidate with the remaining paper seeds." -Params @{ CoursePrereqGate = 0.70 }),
...
} elseif (-not $AllWideVariants) {
    $Variants = @($Variants | Where-Object { $DefaultVariantNames -contains $_.Name })
}
...
function Wait-ForPids {
    ...
    if ($unique.Count -lt 1) {
        Write-QueueLogLine "No active experiment PID to wait for; starting wide-grid multiseed queue."
        return
    }
    Write-QueueLogLine ("Waiting for experiment PIDs before wide-grid multiseed queue: {0}" -f (Format-ListValue $unique))
    while ($true) {
        $alive = @()
...
if ($DryRun) {
    Write-Host "MOOCCube paper-main wide-grid multiseed dry run"
...
Write-QueueLogLine "CONFIG MOOCCube paper-main wide-grid multiseed queue | seeds=$(Format-ListValue $SeedsToRun) | variants=$(Format-ListValue ($Variants.Name)) | baseline=$BaselineRoot | prior=$PriorHparamRoot | output=$OutputRootBase"
...
if (Test-Path -LiteralPath $PriorHparamRootAbs) {
    ...
    Write-QueueLogLine "WARNING prior hparam root missing; continuing with wide-grid roots only: $PriorHparamRootAbs"
}
...
foreach ($variant in $Variants) {
    ...
    Write-QueueLogLine ("START {0} | family={1} | label={2} | missing_seeds={3}" -f $variant.Name, $variant.Family, $variant.Label, (Format-ListValue $missingSeeds))
...
Write-QueueLogLine "All requested MOOCCube paper-main wide-grid multiseed runs finished."
