...
$warmDir = "checkpoints\recppo_research_repair\weightfix_tail03_shared_warmup30_seed2025\strict_item_cold_balanced_thr1_seed_2025"
$stage = Join-Path $warmDir "warmup_stage.pt"
$root = "outputs\recppo_research_repair\weightfix_clean_w100_w150_seed2025"
$ckptRoot = "checkpoints\recppo_research_repair\weightfix_clean_w100_w150_seed2025"
$logRoot = Join-Path $root "logs"
$summary = Join-Path $root "clean_summary.csv"
...
$runs=@(
 [pscustomobject]@{Tag="res006_w100_clean";Weight=1.0},
 [pscustomobject]@{Tag="res006_w150_clean";Weight=1.5}
)
foreach($r in $runs){
 ...
 & .\run_usim_feedback_fast3_content_delta_static.ps1 `
  ...
  -RolloutPolicy ppo -RlResidualScale 0.06 -UsimSteps 5 `
 ...
 $rows=@();foreach($x in $runs){$csv=Join-Path $root "$($x.Tag)\strict_item_cold_balanced_thr1_seed_2025\final_fullrank_usim_feedback_fast3_content_delta_static.csv";if(Test-Path $csv){$d=Import-Csv $csv|Select -First 1;$rows+=[pscustomobject]@{tag=$x.Tag;ppo_weight=$x.Weight;cold_r5=$d.full_cold_item_macro_r5;cold_r10=$d.full_cold_item_macro_r10;cold_n5=$d.full_cold_item_macro_n5;cold_n10=$d.full_cold_item_macro_n10;hot_r5=$d.full_hot_item_macro_r5;hot_r10=$d.full_hot_item_macro_r10;hot_n5=$d.full_hot_item_macro_n5;hot_n10=$d.full_hot_item_macro_n10}}};if($rows.Count){$rows|Export-Csv -NoTypeInformation -Encoding UTF8 $summary}
}
