...
def train_RLmodel(model, train_data, logger, args):
    ...
    for epoch in range(args.max_epoch):
        ...
        logger.logging(
            f"Epoch {epoch + 1}/{args.max_epoch}, total_actor_loss:{total_actor_loss / len(train_data):.4f}, total_critic_loss: {total_critic_loss / len(train_data):.4f}")
...
parser.add_argument('--dataset', type=str, default="MOOCCubeX", help='Dataset to use.')  # ml-1m,CiteULike,XING
...
parser.add_argument('--max_time', type=int, default=3)
parser.add_argument('--transition_rate', type=float, default=0.1)
parser.add_argument('--k', type=int, default=10)
parser.add_argument('--reward_cost', type=float, default=0.01)
...
if 'hybrid_val_user' not in para_dict and 'overall_val_user' in para_dict:
    print("Auto-patching: Mapping 'overall' keys to 'hybrid' keys in RL...")
    para_dict['hybrid_val_user'] = para_dict['overall_val_user']
    para_dict['hybrid_val_user_nb'] = para_dict['overall_val_user_nb']
    para_dict['hybrid_test_user'] = para_dict['overall_test_user']
    para_dict['hybrid_test_user_nb'] = para_dict['overall_test_user_nb']
