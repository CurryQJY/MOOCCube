...
class USIM(torch.nn.Module):
    ...
    def optimize(self, device):
        ...
        with (torch.no_grad()):
            ...
            target_Q = batch_reward.unsqueeze(1) + self.discount * torch.mul(self.target_critic(batch_next_state),
                                                                             (1 - batch_dones))  # 0.99 is the discount
    ...
    def update_buffer(self, interaction, epoch):
        ...
        for i in range(3):  # 采样3轮
            ...
            for i in range(self.max_time):  # 每个item最多生成10个user
                with torch.no_grad():
                    action, mask = self.sample_from_negative_distance6(state, item_embedding, i, state_time,
                                                                       action_list)
        ...
        termination_critic_loss = torch.mean(F.mse_loss(
            self.critic(termination_state),
            torch.zeros((len(item_embedding), 1)).to(self.args.device)
        ))
        self.critic_optimizer.zero_grad()
    ...
    def get_reward(self, target_user, item_embedding, state, action, dones):
        ...
        with torch.no_grad():
            ...
            reward1 = self.weight * (similarity1 - similarity2).squeeze() + (1 - self.weight) * differ_score.mean(
                dim=-1)
    ...
    def infer2(self, content, item_embedding):
        ...
        for i in range(self.max_time):
            with torch.no_grad():
                ...
                reward = torch.cosine_similarity(item_embedding, next_state) - torch.cosine_similarity(item_embedding,
                                                                                                       state)
    ...
    def sample_from_negative_distance6(self, state, target_state, i, state_time, action_list):
        ...
        sim = torch.matmul(distance_vector / torch.norm(distance_vector, p=2, dim=-1, keepdim=True),
                           norm_mat.T).detach()
        ...
        negative_sample = torch.randint(low=0, high=self.n_user - 1, size=((len(state), self.negative_k))).to(
            self.args.device)
