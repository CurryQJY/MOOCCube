...
class ReconstructedOriginalUSIMConfig(CurrentUSIMConfig):
    """
    Reconstructed "original USIM" config.

    Assumption:
    keep the current shared training/eval scaffold (1+200 sampled + full ranking,
    retrieval candidates, early stop), but remove all course-side signals.
    """

    def __init__(self, n_users, n_items, content_dim=768):
        super().__init__(n_users, n_items, content_dim)

        # Disable all course-side behavior.
...
def _save_stream_exports(
    ...
    detail_path = "final_report_usim_original_reconstructed.csv"
    fullrank_path = "final_fullrank_usim_original_reconstructed.csv"
    ...
    fullrank_row = {
        "model": "USIM-Original-Reconstructed",
        "protocol": "stream",
        ...
        "notes": "reconstructed no-course USIM from current usim.py",
    }
...
def main():
    ...
    print(f"Loading Data for reconstructed original USIM from {data_dir}...")
    ...
    cfg = ReconstructedOriginalUSIMConfig(meta["n_users"], meta["n_items"], content_emb.shape[1])
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    model = PAM_RL_Pure_USIM(cfg, content_emb).to(device)
    optimizer = torch.optim.Adam(model.parameters(), lr=cfg.lr)

    print(f">> 架构: Reconstructed Original USIM (no course signals) | Batch Size={cfg.batch_size}")
    print(
    ...
    if use_static:
        print(">> STATIC mode enabled. Reusing current static runner with course flags disabled.")
        ...
        for epoch in range(cfg.n_epochs):
            ...
            if do_early_stop:
                ...
                if is_better:
                    ...
                    es_best_state = {k: v.detach().cpu().clone() for k, v in model.state_dict().items()}
    ...
    print("\n" + "=" * 90)
    print(f"         FINAL REPORT: 采样评估 (1+{cfg.eval_n_neg}) vs 全库排名 (Reconstructed Original USIM)")
    print("=" * 90)
    ...
    metrics_path = "mooc_metrics_pure_usim_reconstructed.csv"
    plot_path = "mooc_result_pure_usim_reconstructed.png"
    ...
    plt.title("Reconstructed Original USIM: Cumulative Training")
    plt.legend()
