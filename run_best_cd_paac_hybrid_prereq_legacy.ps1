param(
    ...
    [string]$OutputDir = "outputs\content_delta_pop5\best_cd_paac_hybrid_prereq_legacy",
    [string]$CkptDir = "checkpoints\content_delta_pop5\best_cd_paac_hybrid_prereq_legacy"
)
...
$env:USIM_STATIC = "0"
$env:USIM_FB_OUTPUT_TAG = "best_cd_paac_hybrid_prereq_legacy"
...
Write-Host "===== Running best_cd_paac_hybrid_prereq_legacy =====" -ForegroundColor Cyan
$commandLine = ('"{0}" -u "{1}" 2>&1' -f $PythonRunner, $ScriptPath)
& cmd.exe /d /c $commandLine | Tee-Object -FilePath $log

if ($LASTEXITCODE -ne 0) {
    throw "Experiment failed: best_cd_paac_hybrid_prereq_legacy"
}
