...
class GraphContentScorer(nn.Module):
    ...
    def propagate(self, adj: torch.Tensor, item_ego: torch.Tensor):
        """LightGCN: L layers over bipartite graph, mean of layers 0..L.

        Returns z_u_all (n_users,d), z_i_all (n_items,d).
        """
...
def main():
    ...
    print(f"n_users={cfg.n_users} n_items={cfg.n_items} content_dim={cfg.content_dim} "
          f"device={device} n_layers={cfg.n_layers}", flush=True)
    ...
    if args.dry_run:
        ...
        print(f"[dry-run] cold items={int((tp < cfg.cold_threshold).sum())}/{cfg.n_items}", flush=True)
