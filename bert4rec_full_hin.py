...
from bert4rec_static_hin import BERT4RecStaticModel
...
class Config:
    def __init__(self, n_users: int, n_items: int, content_dim: int = 768):
        ...
        self.n_epochs = int(os.environ.get("BERT4REC_FULL_EPOCHS", "3"))
        ...
        self.warmup_periods = int(os.environ.get("BERT4REC_FULL_WARMUP", "2"))
        self.period_type = os.environ.get("BERT4REC_PERIOD_TYPE", "M")
        self.use_cumulative = os.environ.get("BERT4REC_USE_CUMULATIVE", "1") == "1"
...
def main():
    ...
    print(f"Model: BERT4Rec stream | device={device}")
    ...
    pd.DataFrame([out]).to_json("bert4rec_full_result.json", orient="records", force_ascii=False)
    print("Saved: bert4rec_full_result.json")
