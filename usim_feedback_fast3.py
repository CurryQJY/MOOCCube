"""
usim_feedback_fast3.py - P2 improvements on top of FAST2
  P2-1. Adaptive target mixing in the USIM gradient update
  P2-2. More stable PPO with GAE, advantage normalization, and value clipping
  P2-3. Soft top-L course rerank instead of fit-only hard sorting
"""
import copy
import json
import math
...
def _feedback_ckpt_dir():
    return os.environ.get("USIM_FB_CKPT_DIR", os.path.join("checkpoints", "usim_feedback_fast3"))
...
def main():
    ...
    print(f">> Architecture: Feedback-Aware RL-USIM + InfoNCE [FAST3] (Batch Size={cfg.batch_size})")
    print(
    ...
    print("\n" + "=" * 90)
    print(f"         FINAL REPORT: sampled (1+{cfg.eval_n_neg}) vs full ranking (RL-USIM FAST3)")
    print("=" * 90)
    print(f"{'Metric':<10} | {'Sampled Cold':<12} | {'Sampled Hot':<12} | {'Full Cold':<12} | {'Full Hot':<12}")
    print("-" * 90)
    summary_rows = []
    sampled_row = {"Model": "USIM-Feedback-FAST3", "Eval": "sampled", "ColdSamples": count_cold, "HotSamples": count_hot}
    full_row = {"Model": "USIM-Feedback-FAST3", "Eval": "full_rank", "ColdSamples": fc_cold, "HotSamples": fc_hot}
    for key in metrics_keys:
    ...
    detail_path, fullrank_path = _save_final_report_exports(
        ...
        model_name="USIM-Feedback-FAST3",
    )

    metrics_path = _feedback_output_path("mooc_metrics_usim_feedback_fast3.csv")
    summary_path = _feedback_output_path("mooc_metrics_usim_feedback_fast3_summary.csv")
    plot_path = _feedback_output_path("mooc_result_usim_feedback_fast3.png")
    ...
    plt.title("RL-USIM [FAST3]: adaptive mix + stable PPO + soft rerank")
    plt.legend()
