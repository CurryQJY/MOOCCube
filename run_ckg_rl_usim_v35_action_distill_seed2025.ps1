...
if ($Seed -ne 2025) {
    throw "This V3.5 viability launcher is pinned to seed 2025. Create a separate launcher for another seed."
}
...
$ScriptPath = "ckg_rl_usim_v35_action_distill.py"
$outputRelative = "outputs\ckg_rl_usim_v35_action_distill\seed$Seed"
$checkpointRelative = "checkpoints\ckg_rl_usim_v35_action_distill\seed$Seed"
if ($RunTag) {
    $outputRelative = "outputs\ckg_rl_usim_v35_action_distill\seed${Seed}_${RunTag}"
    $checkpointRelative = "checkpoints\ckg_rl_usim_v35_action_distill\seed${Seed}_${RunTag}"
}
if ($Smoke) {
    $outputRelative = "outputs\ckg_rl_usim_v35_action_distill\smoke_seed$Seed"
    $checkpointRelative = "checkpoints\ckg_rl_usim_v35_action_distill\smoke_seed$Seed"
    if ($RunTag) {
        $outputRelative = "outputs\ckg_rl_usim_v35_action_distill\smoke_seed${Seed}_${RunTag}"
        $checkpointRelative = "checkpoints\ckg_rl_usim_v35_action_distill\smoke_seed${Seed}_${RunTag}"
    }
}
...
if (-not $DryRun -and ((Test-Path -LiteralPath $outputRoot) -or (Test-Path -LiteralPath $checkpointRoot))) {
    throw "Refusing to overwrite an existing V3.5 run root. Choose a new isolated root."
}
