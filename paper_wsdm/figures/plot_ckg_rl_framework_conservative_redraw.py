...
LAYOUT_SPEC = {
    ...
    "purpose": "Conservative source-of-truth redraw: same modules, icons, variables, arrows, and visual semantics as the provided method figure, with layout cleanup only.",
    "panels": [
        {
        ...
        },
        {
            ...
            "required_modules": [
                ...
                "book and snowflake cold-start symbols",
                "Z_cold",
                "Top-K courses",
            ],
...
def draw_snowflake(ax, cx, cy, r=12, color=None, lw=1.4, z=26):
    color = color or COL["blue"]
    for angle in [0, 60, 120]:
        dx = r * mpl.transforms.Affine2D().rotate_deg(angle).transform((1, 0))[0]
        dy = r * mpl.transforms.Affine2D().rotate_deg(angle).transform((1, 0))[1]
        line(ax, [(cx - dx, cy - dy), (cx + dx, cy + dy)], color=color, lw=lw, z=z)
    for angle in range(0, 360, 60):
        import math

        ex = cx + r * math.cos(math.radians(angle))
        ey = cy + r * math.sin(math.radians(angle))
        for branch in [angle + 35, angle - 35]:
            bx = ex - 4.5 * math.cos(math.radians(branch))
            by = ey - 4.5 * math.sin(math.radians(branch))
            line(ax, [(ex, ey), (bx, by)], color=color, lw=lw, z=z)


def draw_user_icon(ax, cx, cy, color=None, scale=1.0, z=18):
    color = color or COL["line"]
    ax.add_patch(
        patches.Circle(
            (cx, cy),
    ...
    line(ax, [(cx, cy + 14 * scale), (cx, cy + 48 * scale)], color=color, lw=1.9, z=z)
    line(
        ax,
        [(cx - 22 * scale, cy + 62 * scale), (cx, cy + 42 * scale), (cx + 22 * scale, cy + 62 * scale)],
        color=color,
        lw=1.9,
        z=z,
    )
...
def draw_gate_icon(ax, x, y, scale=1.0, color=None, z=34):
    color = color or COL["violet"]
    line(ax, [(x, y + 8 * scale), (x + 28 * scale, y + 8 * scale)], color=color, lw=1.7, z=z)
    line(ax, [(x, y + 22 * scale), (x + 28 * scale, y + 22 * scale)], color=color, lw=1.7, z=z)
    for yy in [8, 22]:
        ax.add_patch(patches.Circle((x - 5 * scale, y + yy * scale), 3.2 * scale, facecolor=COL["paper"], edgecolor=color, linewidth=1.4, zorder=z))
    gate = patches.Polygon(
        [
            (x + 28 * scale, y),
            (x + 58 * scale, y + 15 * scale),
    ...
    ax.add_patch(gate)
    ax.add_patch(patches.Circle((x + 64 * scale, y + 15 * scale), 4.5 * scale, facecolor=COL["paper"], edgecolor=color, linewidth=1.5, zorder=z))
    line(ax, [(x + 68 * scale, y + 15 * scale), (x + 82 * scale, y + 15 * scale)], color=color, lw=1.7, z=z)


def draw_agent_icon(ax, cx, cy, scale=1.0, color=None, z=34):
    color = color or COL["violet"]
    ax.add_patch(
    ...
    ax.add_patch(patches.Circle((cx - 6 * scale, cy - 3 * scale), 3.2 * scale, facecolor=color, edgecolor=color, zorder=z + 1))
    ax.add_patch(patches.Circle((cx + 7 * scale, cy - 3 * scale), 3.2 * scale, facecolor=color, edgecolor=color, zorder=z + 1))
    line(ax, [(cx - 7 * scale, cy + 8 * scale), (cx, cy + 12 * scale), (cx + 8 * scale, cy + 8 * scale)], color=color, lw=1.5, z=z + 1)
    for dx, dy in [(-18, -18), (18, -18), (-18, 18), (18, 18)]:
        line(ax, [(cx, cy), (cx + dx * scale, cy + dy * scale)], color=color, lw=1.0, z=z)


def draw_transition_symbol(ax, cx, cy, scale=1.0, color=None, z=34):
    color = color or COL["line"]
    ax.add_patch(patches.Circle((cx, cy), 15 * scale, facecolor=COL["paper"], edgecolor=color, linewidth=1.5, zorder=z))
    arrow(ax, (cx - 13 * scale, cy - 1 * scale), (cx + 10 * scale, cy - 1 * scale), color=color, lw=1.4, ms=8, z=z + 1, rad=0.25)
    arrow(ax, (cx + 11 * scale, cy + 5 * scale), (cx - 10 * scale, cy + 5 * scale), color=color, lw=1.4, ms=8, z=z + 1, rad=0.25)


def draw_reward_icon(ax, x, y, kind, color):
    if kind == "target":
        ax.add_patch(patches.RegularPolygon((x, y), numVertices=6, radius=9, facecolor="#f7fbfd", edgecolor=color, linewidth=1.6, zorder=22))
        ax.add_patch(patches.Circle((x, y), 3.2, facecolor=color, edgecolor=color, linewidth=1.0, zorder=23))
    elif kind == "progress":
        arrow(ax, (x - 8, y + 4), (x + 7, y - 4), color=color, lw=1.7, ms=8, z=23)
        line(ax, [(x - 8, y - 5), (x + 6, y - 5)], color=color, lw=1.3, z=22)
    elif kind in {"concept", "prereq"}:
        for dx, dy in [(-6, 5), (0, -6), (8, 4)]:
            ax.add_patch(patches.Circle((x + dx, y + dy), 3.2, facecolor=COL["paper"], edgecolor=color, linewidth=1.4, zorder=23))
        line(ax, [(x - 3, y + 3), (x, y - 3), (x + 5, y + 1)], color=color, lw=1.3, z=22)
    elif kind == "difficulty":
        ax.add_patch(patches.Arc((x, y + 5), 20, 15, theta1=200, theta2=340, color=color, linewidth=1.7, zorder=22))
        arrow(ax, (x - 7, y + 5), (x + 5, y + 1), color=color, lw=1.3, ms=7, z=23)
    elif kind == "repeat":
        ax.add_patch(patches.Rectangle((x - 8, y - 8), 16, 16, facecolor=COL["paper"], edgecolor=color, linewidth=1.5, zorder=22))
        line(ax, [(x - 7, y + 7), (x + 7, y - 7)], color=COL["red"], lw=2.0, z=23)
...
def draw_panel_a(ax):
    ...
    draw_snowflake(ax, x + 337, y + 137, r=9, color=COL["blue"], lw=1.3)
    draw_snowflake(ax, x + 276, y + 248, r=8, color=COL["blue"], lw=1.2)
    ...
    draw_gate_icon(ax, fusion_x + 22, fusion_y + 32, scale=0.58, color=COL["violet"])
    text(ax, fusion_x + 105, fusion_y + 28, "fusion MLP + gate", size=9.5, weight="bold", color=COL["violet"], ha="left")
    text(
        ax,
        fusion_x + 58,
        fusion_y + 62,
    ...
    hist_y = y + 768
    draw_user_icon(ax, x + 105, hist_y + 2, color=COL["line"], scale=1.0, z=18)
...
def draw_reward_component(ax, x, y, w, h, title, formula, icon_color):
    ...
    draw_reward_icon(ax, x + 24, y + 29, icon_kind, icon_color)
...
def draw_panel_b(ax):
    ...
    text(ax, sim_x + 72, sim_y + 154, r"$t=0$", size=8.5, weight="bold", color=COL["gold"])
    box(ax, sim_x + 40, sim_y + 126, 130, 35, fc=COL["gold_soft"], ec=COL["gold"], lw=1.6, r=6, z=13)
    text(ax, sim_x + 105, sim_y + 144, r"$t \rightarrow T$", size=8.4, weight="bold", color=COL["gold"])
    ...
    draw_agent_icon(ax, agent_x + 42, agent_y + 50, scale=0.78, color=COL["violet"])
    text(ax, agent_x + 88, agent_y + 32, "actor-critic agent", size=9.8, weight="bold", ha="left")
    text(ax, agent_x + 98, agent_y + 68, r"$\pi_\theta(a \mid s_t, S_t)$", size=8.8, color=COL["muted"], ha="left")
    ...
    draw_transition_symbol(ax, trans_x + 154, trans_y + 103, scale=0.82, color=COL["line"])
    ...
    text(ax, next_x + 34, next_y - 24, r"$t=T$", size=7.8, weight="bold", color=COL["gold"])
...
def draw_panel_c(ax):
    ...
    draw_snowflake(ax, inf_x + 108, inf_y + 45, r=8, color=COL["blue"], lw=1.1, z=20)
    ...
    draw_user_icon(ax, user_x, user_y + 4, color=COL["blue"], scale=0.95, z=18)
    vector_bar(
        ax,
        user_x + 35,
        user_y - 4,
    ...
    draw_snowflake(ax, bank_x - 8, bank_y - 31, r=7, color=COL["blue"], lw=1.0)
    text(ax, bank_x - 16, bank_y - 74, "cold-course bank", size=7.4, weight="bold", color=COL["green"], ha="left")
    text(ax, bank_x - 16, bank_y - 46, "cold bank", size=8.5, weight="bold", color=COL["green"], ha="left")
...
def main():
    ...
    spec_path = spec_dir / "ckg_rl_framework_conservative_redraw_spec.json"
    spec_path.write_text(json.dumps(LAYOUT_SPEC, indent=2), encoding="utf-8")

    fig = build_figure()
    outputs = {
        "svg": out_dir / "ckg_rl_framework_conservative_redraw.svg",
        "pdf": out_dir / "ckg_rl_framework_conservative_redraw.pdf",
        "png": out_dir / "ckg_rl_framework_conservative_redraw.png",
    }
