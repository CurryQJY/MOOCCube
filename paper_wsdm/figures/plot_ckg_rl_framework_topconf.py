...
def state_capsule(ax, x, y, w=42, h=110, label="$s_t$", colors=None, accent=None, band_labels=None, h_label_side="left", z=9):
    ...
    ax.text(
        ...
        vec_y + vec_h / 2 - 18,
        h_label,
        fontsize=10.1 if "t+1" in h_label else 10.8,
        fontweight="bold",
        color=COL["ink"],
    ...
    ax.text(
        ...
        y + h - 6.4,
        tau_label,
        fontsize=7.0 if "t+1" in tau_label else 7.4,
        fontweight="bold",
        color=accent,
        ha="center",
        va="center",
...
def draw_state_transition_function(ax, x, y, w, h, z=13):
    ...
    ax.text(guide_c[0], guide_c[1], "$u_j$", fontsize=7.8, fontweight="bold", color=COL["orange"], ha="center", va="center", zorder=z + 6)
    curved_arrow(
        ax,
        (left_c[0] + 27, y + 52),
        (guide_c[0] - 13, y + 50),
...
def alignment_reward_panel(ax, x, y, w, h, z=14):
    ...
    ax.text(x + w / 2, y + 15, "Emb. Alignment Reward", fontsize=9.2, fontweight="bold", color=COL["ink"], ha="center", va="center", zorder=z + 5)
    ...
    ax.text(c1[0] - 24, c1[1] + 31, "$\\mathbf{e}_c$", fontsize=7.4, fontweight="bold", color=COL["blue2"], ha="center", va="center", zorder=z + 5)
    ax.text(c1[0] + 37, c1[1] - 19, "$\\mathbf{h}_t$", fontsize=7.4, fontweight="bold", color=COL["green"], ha="center", va="center", zorder=z + 5)
    ax.text(c2[0] - 24, c2[1] + 31, "$\\mathbf{e}_c$", fontsize=7.2, fontweight="bold", color=COL["blue2"], ha="center", va="center", zorder=z + 5)
    ax.text(
        c2[0] + 47,
        c2[1] + 30,
        "$\\mathbf{h}_{t+1}$",
...
def course_info_reward_panel(ax, x, y, w, h, z=14):
    ...
    ax.text(x + w / 2, y + 15, "Course-info Reward", fontsize=9.2, fontweight="bold", color=COL["ink"], ha="center", va="center", zorder=z + 5)
    ...
    ax.text(course_cx, course_cy + 25, "$c$", fontsize=8.2, fontweight="bold", color=COL["blue"], ha="center", va="center", zorder=z + 5)
    ...
    for idx, (label, kind, color, fill) in enumerate(chips):
        ...
        ax.text(cx + 36, cy + 15, label, fontsize=7.0, fontweight="bold", color=COL["ink"], ha="left", va="center", zorder=z + 5)


def learner_action_set(ax, x, y, w=226, h=24, z=14):
    box(ax, x, y, w, h, "#fffaf1", ec=COL["line"], lw=1.1, r=1.0, z=z)
    ax.text(x + 60, y + h / 2, "Candidate users:", fontsize=7.5, fontweight="bold", color=COL["ink"], ha="center", va="center", zorder=z + 5)
    ...
    for idx, (label, selected) in enumerate(entries):
        ...
        ax.add_patch(
            patches.Circle(
                (cx, cy),
                8.2 if selected else 7.2,
                facecolor=COL["orange_soft"],
                edgecolor=COL["orange"],
                linewidth=1.25 if selected else 0.9,
                zorder=z + 3,
            )
        )
        ax.text(cx, cy + 0.2, label, fontsize=6.4, fontweight="bold", color=COL["orange"], ha="center", va="center", zorder=z + 4)
    ax.text(x + w - 37, y + h / 2 - 0.2, "$\\cdots$", fontsize=8.5, fontweight="bold", color=COL["muted"], ha="center", va="center", zorder=z + 4)
    x0 = x + w - 15
    y0 = y + h / 2
...
def selected_action_badge(ax, cx, cy, label="$a_t$", z=18):
    ...
    ax.text(cx, cy + 0.1, label, fontsize=8.1, fontweight="bold", color=COL["orange"], ha="center", va="center", zorder=z + 1)
...
def draw_reward_function_reference_style(ax, x, y, w, h, z=13):
    ...
    ax.text(x + align_w + 39, y + h / 2 + 6, "+", fontsize=15, fontweight="bold", color=COL["ink"], ha="center", va="center", zorder=z + 5)
...
def recommendation_row(ax, x, y, w, h, rank, course, score, cold=False, z=6):
    ...
    book_open_icon(ax, x + 31, y + h / 2 + 0.4, size=13, color=COL["line"], lw=0.85, z=z + 3)
    if cold:
        snowflake_icon(ax, x + 36, y + 8, r=2.1, color=COL["ice_dark"], lw=0.55, z=z + 4)
    ax.text(
        x + 44,
        y + h / 2 + 0.2,
        course,
...
def draw_panel_headers(ax):
    ...
    txt(ax, 494, 120, "b", size=17.0, weight="bold", ha="left")
    txt(ax, 870, 121, "Course-knowledge Guided Simulation", size=16.0, weight="bold")
    txt(ax, 1358, 120, "c", size=17.0, weight="bold", ha="left")
    txt(ax, 1549, 121, "Strict Item-Cold Ranking", size=15.0, weight="bold")
...
def draw_left(ax):
    ...
    vector(ax, left_tower_cx - 44, tower_y + 38, w=88, h=15, colors=CONTENT_EMB_COLORS)
    txt(ax, left_tower_cx - 54, tower_y + 46, "$\\mathbf{x}_c$", size=8.4, weight="bold", color=COL["teal"], ha="right")
    mlp_vertical_wedge(ax, left_tower_cx - 32, tower_y + 66, w=64, h=28, ec=COL["teal"], label="MLP", label_size=8.8)
    vector(ax, left_tower_cx - 44, tower_y + 108, w=88, h=16, colors=CONTENT_EMB_COLORS)
    txt(ax, left_tower_cx + 54, tower_y + 116, "$\\mathbf{c}_c$", size=8.4, weight="bold", color=COL["teal"], ha="left")
    ...
    vector(ax, right_tower_cx - 44, tower_y + 38, w=88, h=15, colors=BEHAVIOR_EMB_COLORS)
    txt(ax, right_tower_cx - 54, tower_y + 46, "$\\mathbf{v}_c$", size=8.4, weight="bold", color=COL["muted"], ha="right")
    ...
    vector(ax, right_tower_cx - 44, tower_y + 108, w=88, h=16, colors=BEHAVIOR_EMB_COLORS)
    txt(ax, right_tower_cx + 54, tower_y + 116, "$\\bar{\\mathbf{v}}_c$", size=8.4, weight="bold", color=COL["muted"], ha="left")
...
def draw_middle(ax):
    ...
    txt(ax, 765, 215, "Exploration Set Construction", size=12.2, weight="bold")
    box(ax, 648, 252, 58, 34, COL["card2"], ec=COL["green"], lw=1.3, r=0.5)
    txt(ax, 677, 269, "$\\mathcal{N}_M$", size=10.8, weight="bold", color=COL["green"])
    box(ax, 738, 252, 64, 34, COL["card2"], ec=COL["gold"], lw=1.3, r=0.5)
    txt(ax, 770, 269, "sample $N$", size=9.4, weight="bold", color=COL["gold"])
    ...
    flow_arrow(ax, [(592, 272), (635, 272)], kind="data")
...
def draw_right(ax):
    ...
    snowflake_icon(ax, 1392 + dx, 180, r=6.0, color=COL["ice_dark"], lw=1.0)
    txt(ax, 1535 + dx, 176, "Strict Item-Cold Inference", size=13.2, weight="bold")
    txt(ax, 1535 + dx, 198, "full catalog $\\mathcal{C}_u$", size=11.0, color=COL["muted"])
...
def draw_cross_panel_routes(ax):
    ...
    ax.text(511, 190, "$\\mathrm{init}$", fontsize=10.2, fontweight="bold", color=COL["ink"], ha="center", va="center", zorder=31)
    ax.text(511, 319, "$\\mathbf{e}_c$", fontsize=10.8, fontweight="bold", color=COL["ink"], ha="center", va="center", zorder=31)
