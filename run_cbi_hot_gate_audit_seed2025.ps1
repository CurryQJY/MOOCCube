...
$outputRoot = "outputs\cbi_hot_gate_audit_seed2025"
$checkpointRoot = "checkpoints\cbi_hot_gate_audit_seed2025"
$logRoot = "background_logs\cbi_hot_gate_audit_seed2025"
$manifestPath = Join-Path $outputRoot "run_manifest.json"
$logPath = Join-Path $logRoot "training.log"

$runnerParams = [ordered]@{
    PythonRunner = ".\py.bat"
    ScriptPath = "cbi_hot_gate_audit_seed2025.py"
    ...
    EarlyStopScoreMode = "balanced_rn"
...
$lockedConfig = [ordered]@{
    experiment = "cbi_hot_gate_audit_seed2025"
    method = "hot_only_gate_native_scale_short_checkpoint_audit"
    hot_only_gate = $true
    cold_bypasses_gate = $true
    normalize_hot_fused_before_simulation = $false
    target_anchor = "initial_cbi"
    selection = "balanced_rn_validation_with_epoch_snapshots"
    snapshot_epochs = @(1,2,3,4,5,6,7,8)
...
$sourceFiles = @(
    "run_cbi_hot_gate_audit_seed2025.ps1",
    "cbi_hot_gate_audit_seed2025.py",
    "cbi_anchor_sim.py",
    "cbi_trust_sim.py",
...
if (Test-Path -LiteralPath $manifestPath) { throw "Existing audit manifest found; inspect before restarting: $manifestPath" }
...
$runError = $null
$oldSnapshot = $env:USIM_FB_SNAPSHOT_EPOCHS
try {
    $env:USIM_FB_SNAPSHOT_EPOCHS = "1,2,3,4,5,6,7,8"
    ...
    if ($null -eq $oldSnapshot) { Remove-Item Env:USIM_FB_SNAPSHOT_EPOCHS -ErrorAction SilentlyContinue }
    else { $env:USIM_FB_SNAPSHOT_EPOCHS = $oldSnapshot }
    $timer.Stop()
...
Write-Host "Hot-gate audit seed-2025 experiment completed."
