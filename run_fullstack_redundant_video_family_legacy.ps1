param(
    ...
    [string]$OutputDir = "outputs\content_delta_pop5\fullstack_redundant_video_family_legacy",
    [string]$CkptDir = "checkpoints\content_delta_pop5\fullstack_redundant_video_family_legacy"
)
...
$env:USIM_STATIC = "0"
$env:USIM_FB_OUTPUT_TAG = "fullstack_redundant_video_family_legacy"
...
Write-Host "===== Running fullstack_redundant_video_family_legacy =====" -ForegroundColor Cyan
$commandLine = ('"{0}" -u "{1}" 2>&1' -f $PythonRunner, $ScriptPath)
& cmd.exe /d /c $commandLine | Tee-Object -FilePath $log

if ($LASTEXITCODE -ne 0) {
    throw "Experiment failed: fullstack_redundant_video_family_legacy"
}
