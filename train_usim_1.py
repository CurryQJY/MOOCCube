...
def evaluate(model, loader, device, k_list=[5, 10, 20], eval_type='cold', n_neg=999):
    """
    采样评估：对每个用户，从 1 个正样本 + n_neg 个随机负样本中排名。
    ...
    with torch.no_grad():
        ...
        for start in range(0, n_items, ITEM_BATCH):
            ...
            mask_llm = (llm_batch > -0.5).float().unsqueeze(1)
            val_llm = torch.clamp(llm_batch, min=0.0).unsqueeze(1)
            llm_e = model.llm_proj(val_llm) * mask_llm
            init_state = con_base + llm_e

            final_h, _ = model.run_usim_episode(init_state, None)
            z_i_batch, _ = model.forward_mlp(final_h, model.vars, True)
            all_item_vecs.append(z_i_batch)

        all_item_vecs = torch.cat(all_item_vecs, dim=0)  # [N_items, dim]
        ...
        for batch, pop, llm_s in loader:
            ...
            # 为每个用户采样 n_neg 个负样本
            neg_items = torch.randint(0, n_items, (n_batch, n_neg), device=device)
            # 候选集: [正样本, neg_1, ..., neg_999]，正样本在第 0 位
            ...
            # target 始终在第 0 位
            targets = torch.zeros(n_batch, dtype=torch.long, device=device).view(-1, 1)
            actual_k = min(max(k_list), 1 + n_neg)
...
def main():
    ...
    for t, loader in enumerate(loaders):
        if t >= WARMUP:
            # 评估三类
            met_cold, n_cold = evaluate(model, loader, device, eval_type='cold')
            met_hot, n_hot = evaluate(model, loader, device, eval_type='hot')
            met_all, n_all = evaluate(model, loader, device, eval_type='all')
