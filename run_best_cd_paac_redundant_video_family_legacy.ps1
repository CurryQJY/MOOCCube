param(
    ...
    [string]$OutputDir = "outputs\content_delta_pop5\best_cd_paac_redundant_video_family_legacy",
    [string]$CkptDir = "checkpoints\content_delta_pop5\best_cd_paac_redundant_video_family_legacy",
    [switch]$Resume
)
...
$env:USIM_STATIC = "0"
$env:USIM_FB_OUTPUT_TAG = "best_cd_paac_redundant_video_family_legacy"
...
$env:USIM_USE_CONTENT_DELTA = "1"
$env:USIM_CONTENT_DELTA_MAX_NORM = "0.05"
$env:USIM_CONTENT_DELTA_COLD_ONLY = "1"
$env:USIM_CONTENT_DELTA_SCALE = "0.250"
$env:USIM_CONTENT_DELTA_AUX = "base"
$env:USIM_CONTENT_DELTA_LR_MULT = "0.100"
$env:USIM_CONTENT_DELTA_REG_L2 = "0.020"
$env:USIM_CONTENT_DELTA_REG_CAP = "0.020"
$env:USIM_CONTENT_DELTA_EVAL_BANK = "auto"
$env:USIM_CONTENT_DELTA_NORMALIZE_BASE = "1"
$env:USIM_CONTENT_DELTA_NORMALIZE_OUTPUT = "1"

$env:USIM_USE_PAAC = "1"
$env:USIM_PAAC_ALIGN_W = "0.0"
$env:USIM_PAAC_CONTRAST_W = "0.02"
$env:USIM_PAAC_CONTRAST_BETA = "0.20"
$env:USIM_PAAC_CONTRAST_GAMMA = "0.20"
$env:USIM_PAAC_BATCH_POP_RATIO = "0.50"
$env:USIM_PAAC_ALIGN_MAX_PAIRS = "512"
$env:USIM_PAAC_ALIGN_DETACH_HOT = "1"
...
$env:USIM_FB_COURSE_REDUNDANT_MODE = "video_family"
...
$log = if ($Resume) { Join-Path $OutputDir "run_resume.log" } else { Join-Path $OutputDir "run.log" }
$mode = if ($Resume) { "resume" } else { "fresh" }
Write-Host "===== Running best_cd_paac_redundant_video_family_legacy ($mode) =====" -ForegroundColor Cyan
$commandLine = ('"{0}" -u "{1}" 2>&1' -f $PythonRunner, $ScriptPath)
& cmd.exe /d /c $commandLine | Tee-Object -FilePath $log

if ($LASTEXITCODE -ne 0) {
    throw "Experiment failed: best_cd_paac_redundant_video_family_legacy"
}
